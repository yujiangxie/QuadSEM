...
!  builds the global mass matrix
  ...
  use specfem_par, only: myrank,any_elastic,any_acoustic,any_poroelastic, &
    ...
    rmass_inverse_elastic_m2, & !lucas, CTD-SEM
    ...
    vpext,rhoext,vsext, &
    vpext_m2,rhoext_m2,vsext_m2, & !lucas, CTD-SEM
  ...
  real(kind=CUSTOM_REAL) :: cpl_m2,csl_m2 !lucas, CTD-SEM
  real(kind=CUSTOM_REAL) :: nx,nz,vx,vy,vz,vn,rho_vp,rho_vs,tx,ty,tz, rho_vp_m2,rho_vs_m2,tx_m2,ty_m2,tz_m2, & !lucas, CTD-SEM
  ...
  double precision :: rhol_m2,mul_m2,kappal_relaxed_m2 !lucas, CTD-SEM
  ...
  if (any_elastic) then
    ...
    rmass_inverse_elastic_m2(:,:) = 0._CUSTOM_REAL !lucas1, CTD-SEM
    endif
  endif
    ...
    do j = 1,NGLLZ
      do i = 1,NGLLX
        ...
        if (assign_external_model) then
          ...
          mul_m2 = rhol_m2 * vsext_m2(i,j,ispec)
          endif
          ...
          if (AXISYM) then ! CHECK kappa mm
            ...
            kappal_relaxed_m2 = rhol_m2 * vpext_m2(i,j,ispec)**2 - mul_m2 ! CHECK Kappa
            endif
          endif
        else
        ...
        ! 2.3 lucas
          ...
          if (this_element_has_PML) then
            ...
            if (AXISYM) then
              ...
              !lucas4, CTD-SEM
              if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
              rmass_inverse_elastic_m2(1,iglob) = rmass_inverse_elastic_m2(1,iglob) &
                      + wxgll(i)*wzgll(j)*rhol*jacobian(i,j,ispec)
              endif
            ...
            rmass_inverse_elastic_m2(2,iglob) = rmass_inverse_elastic_m2(1,iglob)
            endif
          endif
        ! 2.4 lucas 
        ...
        if (ispec_is_elastic(ispec)) then
          ...
          if (codeabs(IEDGE4,ispecabs)) then
            ...
            do j = 1,NGLLZ
              ...
              if (P_SV) then
                ...
                tx_m2 = rho_vp_m2*vn*nx+rho_vs_m2*(vx-vn*nx)
                tz_m2 = rho_vp_m2*vn*nz+rho_vs_m2*(vz-vn*nz)

                rmass_inverse_elastic_m2(1,iglob) = rmass_inverse_elastic_m2(1,iglob) + tx_m2*weight
                rmass_inverse_elastic_m2(2,iglob) = rmass_inverse_elastic_m2(2,iglob) + tz_m2*weight 
                endif
                ...
                vy = deltatover2
                ty_m2 = rho_vs_m2*vy
                rmass_inverse_elastic_m2(1,iglob) = rmass_inverse_elastic_m2(1,iglob) + ty_m2*weight
                ! ficticous for SH case, but to be save when inverting
                rmass_inverse_elastic_m2(2,iglob) = rmass_inverse_elastic_m2(1,iglob)
                endif
          ...
          if (codeabs(IEDGE3,ispecabs)) then
            ...
            do i = 1,NGLLX
              ...
              if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas15, CTD-SEM-----
               if (assign_external_model) then
                cpl_m2 = vpext_m2(i,j,ispec)
                csl_m2 = vsext_m2(i,j,ispec)
                rhol_m2 = rhoext_m2(i,j,ispec)
              ...
              if (P_SV) then
                ...
                if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas16, CTD-SEM
                tx_m2 = rho_vp_m2*vn*nx+rho_vs_m2*(vx-vn*nx)
                tz_m2 = rho_vp_m2*vn*nz+rho_vs_m2*(vz-vn*nz)
                rmass_inverse_elastic_m2(1,iglob) = rmass_inverse_elastic_m2(1,iglob) + tx_m2*weight
                rmass_inverse_elastic_m2(2,iglob) = rmass_inverse_elastic_m2(2,iglob) + tz_m2*weight
                ...
                if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas17, CTD-SEM
                vy = deltatover2
                ty_m2 = rho_vs_m2*vy
                rmass_inverse_elastic_m2(1,iglob) = rmass_inverse_elastic_m2(1,iglob) + ty_m2*weight
                ! ficticous for SH case, but to be save when inverting
...
! inverts the global mass matrix
  ...
  use specfem_par, only: myrank,any_elastic,any_acoustic,any_poroelastic, &
                                rmass_inverse_elastic, &
                                rmass_inverse_elastic_m2, & !lucas, CTD_SEM
...
! (this can happen when some degrees of freedom have been removed from some of the global arrays)
  if (any_elastic) then
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
    where(rmass_inverse_elastic_m2 <= 0._CUSTOM_REAL) rmass_inverse_elastic_m2 = 1._CUSTOM_REAL
    endif
  endif
...
! compute the inverse of the mass matrix
  if (any_elastic) then
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
    rmass_inverse_elastic_m2(:,:) = 1._CUSTOM_REAL / rmass_inverse_elastic_m2(:,:) ! lucas, CTD-SEM
    endif
  endif
