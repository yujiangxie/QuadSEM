...
!========================================================================
  ...
  mu = poroelastcoef(2,1,numat_local) ! lucas: from tomo if using external tomo. In case of indic=1, mu=rho*vs*vs, and lambdaplus2mu=rho*vp*vp, and denst=rho.
  lambdaplus2mu  = poroelastcoef(3,1,numat_local)
  denst = density(1,numat_local)

  cploc = sqrt(lambdaplus2mu/denst) ! lucas:cploc=sqrt(rho*vp*vp/rho)
  csloc = sqrt(mu/denst) ! lucas: csloc=sqrt(rho*vs*vs/rho)

  ! plane wave type
  if (source_type(1) == 1 .or. source_type(1) == 4) then

    ! P wave case
    p = sin(anglesource_abs)/cploc ! lucas: p=sin(anglesource_abs)/vp;
    c_inc  = cploc ! lucas: c_inc=vp
    c_refl = csloc ! lucas:c_refl=vs

    anglesource_refl = asin(p*c_refl) ! lucas: sin(anglesource_refl)=sin(anglesource_abs)*vs/vp;
  ...
  if (.not. over_critical_angle) then 
    ...
    do i = 1,nglob
      ...
      t = 0.d0 + time_offset ! lucas, time_offset= -t0

      ! formulas for the initial displacement for a plane wave from Aki & Richards (1980) 
      !lucas: in ricker_Bielak_displ(t,f0), ricker_Bielak_displ = (1 - 2*a*t**2)*exp(-a*t**2), and a = pi*pi*f0*f0, see compute_Bielak_conditions.f90
      displ_elastic(1,i) = &
