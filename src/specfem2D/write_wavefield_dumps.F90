...
! To avoid warnings by the compiler about unused variables in case of a serial code.
  ...
  if (this_is_the_first_time_we_dump) then
    ...
    if (NPROC > 1) then
      ...
      allocate(dump_write(2, size(dump_recv,2))) ! lucas, DIM=2, and dump_recv(x or y,iglob), i.e., the size of iglob. Meaning that we have the numder of iglob for dump_write()
      dump_write(:,:) = dump_recv(:,1:icounter) ! lucas: icounter=ispec*NGLLX*NGLLZ
    endif ! if (NPROC > 1)
#else
  ...
  if (imagetype_wavefield_dumps == 1) then
    ...
    call compute_vector_whole_medium(potential_acoustic,displ_elastic,displs_poroelastic) ! lucas, got the vector_field_display, the same also for velocity and acceleration below
  ...
  contains
    ...
    ! Write file content
    do ii=1, size(dump_write, 2) ! lucas, size(dump_write, 2)=the number of iglob.
