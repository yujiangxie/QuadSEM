...
!         endif
  ...
  !lucas, for m2, CTD-SEM, 
  subroutine compute_attenuation_viscoelastic_m2(e1,e11,e13,dux_dxl,dux_dzl,duz_dxl,duz_dzl,dux_dxl_old,duz_dzl_old, &
                                              dux_dzl_plus_duz_dxl_old,PML_BOUNDARY_CONDITIONS,i,j,ispec, &
                                              e1_sum,e11_sum,e13_sum)
  ...
  use specfem_par, only: nspec_ATT_el,ATTENUATION_VISCOELASTIC,N_SLS, &
                         ispec_is_PML, &
                         inv_tau_sigma_nu1_m2,phi_nu1_m2,inv_tau_sigma_nu2_m2,phi_nu2_m2,time_stepping_scheme,i_stage,deltat, &
                         e1_LDDRK_m2,e11_LDDRK_m2,e13_LDDRK_m2,e1_initial_rk_m2,e11_initial_rk_m2,e13_initial_rk_m2, &
                         e1_force_RK_m2,e11_force_RK_m2,e13_force_RK_m2, & 
                         A_newmark_nu1_m2,B_newmark_nu1_m2,A_newmark_nu2_m2,B_newmark_nu2_m2
...
! (tests made by Ting Yu and also by Zhinan Xie, CNRS Marseille, France, show that it is better to leave it to .true.)
  ...
  if (PML_BOUNDARY_CONDITIONS .and. ispec_is_PML(ispec)) return
  ! convention to indicate that Q = 9999 in that element i.e. that there is no viscoelasticity at that GLL point
  if (inv_tau_sigma_nu1_m2(i,j,ispec,1) < 0.) return
  ...
  do i_sls = 1,N_SLS
    ...
    if (time_stepping_scheme /= 1) then !lucas, CTD-SEM
      phinu1 = phi_nu1_m2(i,j,ispec,i_sls)
      tauinvnu1 = inv_tau_sigma_nu1_m2(i,j,ispec,i_sls)
      phinu2 = phi_nu2_m2(i,j,ispec,i_sls)
      tauinvnu2 = inv_tau_sigma_nu2_m2(i,j,ispec,i_sls)
    endif
...
!         if (CONVOLUTION_MEMORY_VARIABLES) then
!! DK DK inlined this for speed            call compute_coef_convolution(tauinvnu1,deltat,coef0,coef1,coef2)
        a_newmark = A_newmark_nu1_m2(i_sls,i,j,ispec)
        e1(i_sls,i,j,ispec) = a_newmark * a_newmark * e1(i_sls,i,j,ispec) + &
                              B_newmark_nu1_m2(i_sls,i,j,ispec) * (theta_n_u + a_newmark * theta_nsub1_u)

!! DK DK inlined this for speed            call compute_coef_convolution(tauinvnu2,deltat,coef0,coef1,coef2)
        a_newmark = A_newmark_nu2_m2(i_sls,i,j,ispec)
        e11(i_sls,i,j,ispec) = a_newmark * a_newmark * e11(i_sls,i,j,ispec) + &
                               B_newmark_nu2_m2(i_sls,i,j,ispec) * (dux_dxl-theta_n_u/TWO + &
                               a_newmark * (dux_dxl_old(i,j,ispec)-theta_nsub1_u/TWO))

        e13(i_sls,i,j,ispec) = a_newmark * a_newmark * e13(i_sls,i,j,ispec) + &
                               B_newmark_nu2_m2(i_sls,i,j,ispec) * (dux_dzl + duz_dxl + &
                               a_newmark * (dux_dzl_plus_duz_dxl_old(i,j,ispec)))
...
!         endif
    ...
    case (2)
      ! LDDRK
      ! update e1, e11, e13 in ADE formation with fourth-order LDDRK scheme
      e1_LDDRK_m2(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e1_LDDRK_m2(i,j,ispec,i_sls) + &
                                  deltat * (theta_n_u * phinu1 - e1(i_sls,i,j,ispec) * tauinvnu1)
      e1(i_sls,i,j,ispec) = e1(i_sls,i,j,ispec) + BETA_LDDRK(i_stage) * e1_LDDRK_m2(i,j,ispec,i_sls)

      e11_LDDRK_m2(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e11_LDDRK_m2(i,j,ispec,i_sls) + &
                                   deltat * ((dux_dxl-theta_n_u/TWO) * phinu2) - &
                                   deltat * (e11(i_sls,i,j,ispec) * tauinvnu2)
      e11(i_sls,i,j,ispec) = e11(i_sls,i,j,ispec)+BETA_LDDRK(i_stage)*e11_LDDRK_m2(i,j,ispec,i_sls)

      e13_LDDRK_m2(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e13_LDDRK_m2(i,j,ispec,i_sls) + &
                                   deltat * ((dux_dzl + duz_dxl)*phinu2) - &
                                   deltat * (e13(i_sls,i,j,ispec) * tauinvnu2)
      e13(i_sls,i,j,ispec) = e13(i_sls,i,j,ispec)+BETA_LDDRK(i_stage) * e13_LDDRK_m2(i,j,ispec,i_sls)

    case (3)
      ! Runge-Kutta
      ! update e1, e11, e13 in ADE formation with classical fourth-order Runge-Kutta scheme
      e1_force_RK_m2(i,j,ispec,i_sls,i_stage) = deltat * (theta_n_u * phinu1 - e1(i_sls,i,j,ispec) * tauinvnu1)

      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        if (i_stage == 1) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 2) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 3) weight_rk = 1._CUSTOM_REAL
        if (i_stage == 1) e1_initial_rk_m2(i,j,ispec,i_sls) = e1(i_sls,i,j,ispec)
        e1(i_sls,i,j,ispec) = e1_initial_rk_m2(i,j,ispec,i_sls) + weight_rk * e1_force_RK_m2(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e1(i_sls,i,j,ispec) = e1_initial_rk_m2(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                              (e1_force_RK_m2(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e1_force_RK_m2(i,j,ispec,i_sls,2) + &
                               2._CUSTOM_REAL * e1_force_RK_m2(i,j,ispec,i_sls,3) + e1_force_RK_m2(i,j,ispec,i_sls,4))
      endif

      e11_force_RK_m2(i,j,ispec,i_sls,i_stage) = deltat * ((dux_dxl-theta_n_u/TWO) * phinu2 - &
                                                         e11(i_sls,i,j,ispec) * tauinvnu2)
      ...
      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        ...
        if (i_stage == 1) e11_initial_rk_m2(i,j,ispec,i_sls) = e11(i_sls,i,j,ispec)
        e11(i_sls,i,j,ispec) = e11_initial_rk_m2(i,j,ispec,i_sls) + weight_rk * e11_force_RK_m2(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e11(i_sls,i,j,ispec) = e11_initial_rk_m2(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                               (e11_force_RK_m2(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e11_force_RK_m2(i,j,ispec,i_sls,2) + &
                                2._CUSTOM_REAL * e11_force_RK_m2(i,j,ispec,i_sls,3) + e11_force_RK_m2(i,j,ispec,i_sls,4))
      endif

      e13_force_RK_m2(i,j,ispec,i_sls,i_stage) = deltat * ((dux_dzl + duz_dxl)*phinu2 - &
                                                         e13(i_sls,i,j,ispec) * tauinvnu2)
      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        if (i_stage == 1) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 2) weight_rk = 0.5_CUSTOM_REAL
        ...
        if (i_stage == 1) e13_initial_rk_m2(i,j,ispec,i_sls) = e13(i_sls,i,j,ispec)
        e13(i_sls,i,j,ispec) = e13_initial_rk_m2(i,j,ispec,i_sls) + weight_rk * e13_force_RK_m2(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e13(i_sls,i,j,ispec) = e13_initial_rk_m2(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                               (e13_force_RK_m2(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e13_force_RK_m2(i,j,ispec,i_sls,2) + &
                                2._CUSTOM_REAL * e13_force_RK_m2(i,j,ispec,i_sls,3) + e13_force_RK_m2(i,j,ispec,i_sls,4))
      endif
  ...
  end subroutine compute_attenuation_viscoelastic_m2



  !lucas, for m1, CTD-SEM, using in computing approximate Hessian kernels, that is engine C in the Hessian kernels GJI paper, 2021.
  subroutine compute_attenuation_viscoelastic_m1(e1,e11,e13,dux_dxl,dux_dzl,duz_dxl,duz_dzl,dux_dxl_old,duz_dzl_old, &
                                              dux_dzl_plus_duz_dxl_old,PML_BOUNDARY_CONDITIONS,i,j,ispec, &
                                              e1_sum,e11_sum,e13_sum)
  ...
  use specfem_par, only: nspec_ATT_el,ATTENUATION_VISCOELASTIC,N_SLS, &  !lucas, CTD-SEM, only need to check these, related to specfem_par
                         ispec_is_PML, &
                         inv_tau_sigma_nu1_m1,phi_nu1_m1,inv_tau_sigma_nu2_m1,phi_nu2_m1,time_stepping_scheme,i_stage,deltat, &
                         e1_LDDRK_m1,e11_LDDRK_m1,e13_LDDRK_m1,e1_initial_rk_m1,e11_initial_rk_m1,e13_initial_rk_m1, &
                         e1_force_RK_m1,e11_force_RK_m1,e13_force_RK_m1, & 
                         A_newmark_nu1_m1,B_newmark_nu1_m1,A_newmark_nu2_m1,B_newmark_nu2_m1 
...
! (tests made by Ting Yu and also by Zhinan Xie, CNRS Marseille, France, show that it is better to leave it to .true.)
  ...
  if (PML_BOUNDARY_CONDITIONS .and. ispec_is_PML(ispec)) return
  ! convention to indicate that Q = 9999 in that element i.e. that there is no viscoelasticity at that GLL point
  if (inv_tau_sigma_nu1_m1(i,j,ispec,1) < 0.) return
  ...
  do i_sls = 1,N_SLS
    ...
    if (time_stepping_scheme /= 1) then !lucas, CTD-SEM
      phinu1 = phi_nu1_m1(i,j,ispec,i_sls)
      tauinvnu1 = inv_tau_sigma_nu1_m1(i,j,ispec,i_sls)
      phinu2 = phi_nu2_m1(i,j,ispec,i_sls)
      tauinvnu2 = inv_tau_sigma_nu2_m1(i,j,ispec,i_sls)
    endif
...
!         if (CONVOLUTION_MEMORY_VARIABLES) then
!! DK DK inlined this for speed            call compute_coef_convolution(tauinvnu1,deltat,coef0,coef1,coef2)
        a_newmark = A_newmark_nu1_m1(i_sls,i,j,ispec)
        e1(i_sls,i,j,ispec) = a_newmark * a_newmark * e1(i_sls,i,j,ispec) + &
                              B_newmark_nu1_m1(i_sls,i,j,ispec) * (theta_n_u + a_newmark * theta_nsub1_u)

!! DK DK inlined this for speed            call compute_coef_convolution(tauinvnu2,deltat,coef0,coef1,coef2)
        a_newmark = A_newmark_nu2_m1(i_sls,i,j,ispec)
        e11(i_sls,i,j,ispec) = a_newmark * a_newmark * e11(i_sls,i,j,ispec) + &
                               B_newmark_nu2_m1(i_sls,i,j,ispec) * (dux_dxl-theta_n_u/TWO + &
                               a_newmark * (dux_dxl_old(i,j,ispec)-theta_nsub1_u/TWO))

        e13(i_sls,i,j,ispec) = a_newmark * a_newmark * e13(i_sls,i,j,ispec) + &
                               B_newmark_nu2_m1(i_sls,i,j,ispec) * (dux_dzl + duz_dxl + &
                               a_newmark * (dux_dzl_plus_duz_dxl_old(i,j,ispec)))
...
!         endif
    ...
    case (2)
      ! LDDRK
      ! update e1, e11, e13 in ADE formation with fourth-order LDDRK scheme
      e1_LDDRK_m1(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e1_LDDRK_m1(i,j,ispec,i_sls) + &
                                  deltat * (theta_n_u * phinu1 - e1(i_sls,i,j,ispec) * tauinvnu1)
      e1(i_sls,i,j,ispec) = e1(i_sls,i,j,ispec) + BETA_LDDRK(i_stage) * e1_LDDRK_m1(i,j,ispec,i_sls)

      e11_LDDRK_m1(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e11_LDDRK_m1(i,j,ispec,i_sls) + &
                                   deltat * ((dux_dxl-theta_n_u/TWO) * phinu2) - &
                                   deltat * (e11(i_sls,i,j,ispec) * tauinvnu2)
      e11(i_sls,i,j,ispec) = e11(i_sls,i,j,ispec)+BETA_LDDRK(i_stage)*e11_LDDRK_m1(i,j,ispec,i_sls)

      e13_LDDRK_m1(i,j,ispec,i_sls) = ALPHA_LDDRK(i_stage) * e13_LDDRK_m1(i,j,ispec,i_sls) + &
                                   deltat * ((dux_dzl + duz_dxl)*phinu2) - &
                                   deltat * (e13(i_sls,i,j,ispec) * tauinvnu2)
      e13(i_sls,i,j,ispec) = e13(i_sls,i,j,ispec)+BETA_LDDRK(i_stage) * e13_LDDRK_m1(i,j,ispec,i_sls)

    case (3)
      ! Runge-Kutta
      ! update e1, e11, e13 in ADE formation with classical fourth-order Runge-Kutta scheme
      e1_force_RK_m1(i,j,ispec,i_sls,i_stage) = deltat * (theta_n_u * phinu1 - e1(i_sls,i,j,ispec) * tauinvnu1)

      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        if (i_stage == 1) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 2) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 3) weight_rk = 1._CUSTOM_REAL
        if (i_stage == 1) e1_initial_rk_m1(i,j,ispec,i_sls) = e1(i_sls,i,j,ispec)
        e1(i_sls,i,j,ispec) = e1_initial_rk_m1(i,j,ispec,i_sls) + weight_rk * e1_force_RK_m1(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e1(i_sls,i,j,ispec) = e1_initial_rk_m1(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                              (e1_force_RK_m1(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e1_force_RK_m1(i,j,ispec,i_sls,2) + &
                               2._CUSTOM_REAL * e1_force_RK_m1(i,j,ispec,i_sls,3) + e1_force_RK_m1(i,j,ispec,i_sls,4))
      endif

      e11_force_RK_m1(i,j,ispec,i_sls,i_stage) = deltat * ((dux_dxl-theta_n_u/TWO) * phinu2 - &
                                                         e11(i_sls,i,j,ispec) * tauinvnu2)
      ...
      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        ...
        if (i_stage == 1) e11_initial_rk_m1(i,j,ispec,i_sls) = e11(i_sls,i,j,ispec)
        e11(i_sls,i,j,ispec) = e11_initial_rk_m1(i,j,ispec,i_sls) + weight_rk * e11_force_RK_m1(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e11(i_sls,i,j,ispec) = e11_initial_rk_m1(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                               (e11_force_RK_m1(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e11_force_RK_m1(i,j,ispec,i_sls,2) + &
                                2._CUSTOM_REAL * e11_force_RK_m1(i,j,ispec,i_sls,3) + e11_force_RK_m1(i,j,ispec,i_sls,4))
      endif

      e13_force_RK_m1(i,j,ispec,i_sls,i_stage) = deltat * ((dux_dzl + duz_dxl)*phinu2 - &
                                                         e13(i_sls,i,j,ispec) * tauinvnu2)
      if (i_stage == 1 .or. i_stage == 2 .or. i_stage == 3) then
        if (i_stage == 1) weight_rk = 0.5_CUSTOM_REAL
        if (i_stage == 2) weight_rk = 0.5_CUSTOM_REAL
        ...
        if (i_stage == 1) e13_initial_rk_m1(i,j,ispec,i_sls) = e13(i_sls,i,j,ispec)
        e13(i_sls,i,j,ispec) = e13_initial_rk_m1(i,j,ispec,i_sls) + weight_rk * e13_force_RK_m1(i,j,ispec,i_sls,i_stage)
      else if (i_stage == 4) then
        e13(i_sls,i,j,ispec) = e13_initial_rk_m1(i,j,ispec,i_sls) + 1._CUSTOM_REAL / 6._CUSTOM_REAL * &
                               (e13_force_RK_m1(i,j,ispec,i_sls,1) + 2._CUSTOM_REAL * e13_force_RK_m1(i,j,ispec,i_sls,2) + &
                                2._CUSTOM_REAL * e13_force_RK_m1(i,j,ispec,i_sls,3) + e13_force_RK_m1(i,j,ispec,i_sls,4))
      endif
  ...
  end subroutine compute_attenuation_viscoelastic_m1
