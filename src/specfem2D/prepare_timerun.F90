...
!========================================================================
  ...
  call prepare_wavefields() ! lucas: prepare_wavefields(), i.e., allocate displ_elastic, veloc_elastic, accel_elastic
  ...
  if (setup_with_binary_database /= 2) then
    ...
    call prepare_timerun_adjoint()!lucas,prepare_timerun_kernels() here, also initialzie rho_kl(:,:,:) = 0._CUSTOM_REAL, etc.

    ! reads initial fields from external file if needed
    call prepare_timerun_initialfield ! lucas: for plane wave

    ! compute the source time function and stores it in a text file
    call prepare_source_time_function() ! lucas: about 11 typies of source time function
...
!
  ...
  allocate(sisux_m2(NSTEP_BETWEEN_OUTPUT_SEISMOS/subsamp_seismos,nrecloc,NSIGTYPE), &
           sisuz_m2(NSTEP_BETWEEN_OUTPUT_SEISMOS/subsamp_seismos,nrecloc,NSIGTYPE), &
           siscurl_m2(NSTEP_BETWEEN_OUTPUT_SEISMOS/subsamp_seismos,nrecloc,NSIGTYPE),stat=ier)
  if (ier /= 0) call stop_the_code('Error allocating seismogram arrays for m2 in CTD-SEM')

  sisux_m2(:,:,:) = ZERO ! double precision zero
  sisuz_m2(:,:,:) = ZERO
  siscurl_m2(:,:,:) = ZERO
  endif !--------------------------------------
...
!
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
    call assemble_MPI_scalar_m2(rmass_inverse_acoustic,nglob_acoustic, &
                           rmass_inverse_e1,n_sls_loc, &
                           rmass_inverse_elastic_m2, nglob_elastic, &  !lucas, CTD-SEM
                           rmass_s_inverse_poroelastic,rmass_w_inverse_poroelastic,nglob_poroelastic)
...
!
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
   allocate(vector_field_display_m2(NDIM,nglob))
   vector_field_display_m2(:,:) = 0.d0
   if(compute_appro_Hessian) then
      allocate(vector_field_display_m1(NDIM,nglob))
      vector_field_display_m1(:,:) = 0.d0
   endif
  endif
...
!
  ...
  if (output_color_image) then
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
     allocate(image_color_data_m2(NX_IMAGE_color,NZ_IMAGE_color),stat=ier)
     if (ier /= 0) call stop_the_code('error in an allocate statement 1 for m2 in CTD-SEM')
     if(compute_appro_Hessian) then
        allocate(image_color_data_m1(NX_IMAGE_color,NZ_IMAGE_color),stat=ier)
        if (ier /= 0) call stop_the_code('error in an allocate statement 1 for m1 for approximate Hessian')
     endif
    endif
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
     allocate(image_color_vp_display_m2(NX_IMAGE_color,NZ_IMAGE_color),stat=ier)
     if (ier /= 0) call stop_the_code('error in an allocate statement 2 for m2 in CTD-SEM')
     if(compute_appro_Hessian) then !---
      allocate(image_color_vp_display_m1(NX_IMAGE_color,NZ_IMAGE_color),stat=ier)
      if (ier /= 0) call stop_the_code('error in an allocate statement 2 for m1 for approximate Hessian')
     endif !---
    endif
    ...
    if (myrank == 0) then
      ...
      if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
        allocate(data_pixel_recv_m2(maxval(nb_pixel_per_proc(:))))
        if(compute_appro_Hessian) then
          allocate(data_pixel_recv_m1(maxval(nb_pixel_per_proc(:))))
        endif
      endif
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
     allocate(data_pixel_send_m2(nb_pixel_loc))
     if(compute_appro_Hessian) then
       allocate(data_pixel_send_m1(nb_pixel_loc))
     endif
    endif
    ...
    call prepare_color_image_vp() !lucas, CTD-SEM
...
! prepares adjoint runs
  ...
  if (SIMULATION_TYPE == 3) then
    ...
    call prepare_timerun_kernels() !lucas2, CTD-SEM, already done for elastic and save_ASCII_kernels
...
!
  ...
  subroutine prepare_timerun_kernels() !lucas, include CTD-SEM
  ...
  if (any_elastic) then
    ...
    if (save_ASCII_kernels) then
      ...
      if (count(ispec_is_anisotropic(:) .eqv. .true.) >= 1) then ! anisotropic
        ...
        !Ha
        if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas,CTD-SEM---------
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_Ha.dat'
        open(unit = 97+100, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) 
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        !Hbm
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_Hbm.dat'
        open(unit = 97+101, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) 
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        !Hbs
        if(compute_appro_Hessian) then 
          write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_Hbs.dat'
          open(unit = 97+101+1000, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) 
          if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        endif
        !Hc
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_Hc.dat' 
        open(unit = 97+102, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) 
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')  
        !Habc
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_Habc.dat' 
        open(unit = 97+103, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) 
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        endif !-----------------------------------------------------------------------
        ...
        !Ha
        if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas,CTD-SEM-------------
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_Ha.dat'
        open(unit = 98+200, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier)
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        !Hbm
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_Hbm.dat'
        open(unit = 98+201, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier)
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        !Hbs
        if(compute_appro_Hessian) then
          write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_Hbs.dat'
          open(unit = 98+201+1000, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier)
          if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        endif
        !Hc
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_Hc.dat'
        open(unit = 98+202, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier)
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        !Habc
        write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_Habc.dat'
        open(unit = 98+203, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier)
        if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        endif !-------------------------------------------------------------------------

        ! add by lucas --------------
        if (APPROXIMATE_HESS_KL) then !lucas, not used
          write(outputname,'(a,i6.6,a)') 'proc',myrank,'_Approxi_Hessian1_kernel.dat' ! lucas
          open(unit =214,file=trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) ! lucas
          if (ier /= 0) call stop_the_code('Error writing kernel file to disk')

          write(outputname,'(a,i6.6,a)') 'proc',myrank,'_Approxi_Hessian2_kernel.dat' ! lucas
          open(unit=215,file=trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) !lucas
          if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
        endif
        ! lucas --------------------
      endif !lucas, end ispec_is_aniso

 !     if(CTD_SEM) then !lucas, CTD-SEM---------------start----------------------- used for test only, not used
 !     if (count(ispec_is_anisotropic(:) .eqv. .true.) >= 1) then ! anisotropic
 !       write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_cijkl_kernel_m2.dat'
 !       open(unit = 970, file=trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) !lucas, need to do for 970 in save
 !       if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
 !     else
 !       write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rho_kappa_mu_kernel_m2.dat' 
 !       open(unit = 970, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) !lucas, need to do for 970 in save
 !       if (ier /= 0) call stop_the_code('Error writing kernel file to disk')

 !       write(outputname,'(a,i6.6,a)') 'proc',myrank,'_rhop_alpha_beta_kernel_m2.dat'
 !       open(unit = 980, file = trim(OUTPUT_FILES)//outputname,status='unknown',iostat=ier) !lucas, need to do for 980 in save
 !       if (ier /= 0) call stop_the_code('Error writing kernel file to disk')
 !     endif !lucas, end ispec_is_aniso
 !     endif!-------------------------end CTD-SEM-----------------------------------
    ...
    if(compute_appro_Hessian) then !Hbs
    ...
    !!----rhop, beta, alpha
    rhop_kl_Ha(:,:,:) = 0._CUSTOM_REAL
    beta_kl_Ha(:,:,:) = 0._CUSTOM_REAL
    alpha_kl_Ha(:,:,:) = 0._CUSTOM_REAL
    !--------Hbm
    rhop_kl_Hbm(:,:,:) = 0._CUSTOM_REAL
    beta_kl_Hbm(:,:,:) = 0._CUSTOM_REAL
    alpha_kl_Hbm(:,:,:) = 0._CUSTOM_REAL
    if(compute_appro_Hessian) then !Hbs
      rhop_kl_Hbs(:,:,:) = 0._CUSTOM_REAL
      beta_kl_Hbs(:,:,:) = 0._CUSTOM_REAL
      alpha_kl_Hbs(:,:,:) = 0._CUSTOM_REAL
    endif
    !--------
    rhop_kl_Hc(:,:,:) = 0._CUSTOM_REAL
    beta_kl_Hc(:,:,:) = 0._CUSTOM_REAL
    alpha_kl_Hc(:,:,:) = 0._CUSTOM_REAL
    !--------
    rhop_kl_Habc(:,:,:) = 0._CUSTOM_REAL
    beta_kl_Habc(:,:,:) = 0._CUSTOM_REAL
    alpha_kl_Habc(:,:,:) = 0._CUSTOM_REAL
    endif !lucas, end CTD-SEM-------------------------------------------------------------------

   ! if(for_test_m2) then!-------lucas-
   ! rho_kl_m2(:,:,:) = 0._CUSTOM_REAL
   ! mu_kl_m2(:,:,:) = 0._CUSTOM_REAL
   ! kappa_kl_m2(:,:,:) = 0._CUSTOM_REAL

   ! rhop_kl_m2(:,:,:) = 0._CUSTOM_REAL
   ! beta_kl_m2(:,:,:) = 0._CUSTOM_REAL
   ! alpha_kl_m2(:,:,:) = 0._CUSTOM_REAL
   ! endif!-------------------------
...
! reads initial fields from external file if needed
  ...
  if (any_elastic) then
    ...
    call prepare_initial_field(cploc,csloc) ! lucas: got (PP PS)/(SS SP) A_plane(:) B_plane(:) C_plane(:) x0_source z0_source, xmin xmax zmin zmax
...
!
  ...
  double precision :: vp_m2,vs_m2,rhol_m2,mul_m2,kappal_m2 !lucas, CTD-SEM
  ...
  !lucas, CTD-SEM
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
  allocate(kappastore_m2(NGLLX,NGLLZ,nspec), &
           mustore_m2(NGLLX,NGLLZ,nspec), &
           rhostore_m2(NGLLX,NGLLZ,nspec), &
           rho_vp_m2(NGLLX,NGLLZ,nspec), &
           rho_vs_m2(NGLLX,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call stop_the_code('Error allocating material arrays for m2 in CTD-SEM')
  endif
    ...
    do j = 1,NGLLZ
      do i = 1,NGLLX
        if (assign_external_model) then
          ...
          vp_m2 = vpext_m2(i,j,ispec)
          vs_m2 = vsext_m2(i,j,ispec)
          ! determins mu and kappa
          mul_m2 = rhol_m2 * vs_m2 * vs_m2
          endif 
          ...
          if (AXISYM) then ! CHECK kappa
            ...
            if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
            kappal_m2 = rhol_m2 * vp_m2 * vp_m2 - mul_m2
        ...
        if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM
        rhostore_m2(i,j,ispec) = rhol_m2
        mustore_m2(i,j,ispec) = mul_m2
        kappastore_m2(i,j,ispec) = kappal_m2
        ! stores density times vp and vs
        vs_m2 = sqrt(mul_m2/rhol_m2)
        rho_vp_m2(i,j,ispec) = rhol_m2 * vp_m2
        rho_vs_m2(i,j,ispec) = rhol_m2 * vs_m2
        endif
