...
!------------------------------------------------------------------------------------------------
  ...
  ! elastic domain
  if (ELASTIC_SIMULATION) call update_displ_elastic_forward() !lucas, include m2, and m1 (for approximate Hessian)
...
!
  ...
  subroutine update_displ_Newmark_backward() !lucas, only used for forward reconstruction back in time
...
! visco-elastic domains
  ...
  if (.not. GPU_MODE) then
   ...
   !Lucas, CTD-SEM, in case of coupled_acoustic_elastic, something need to be implememted here
   !----
   ! to be implemnetd for coupled_acoustic_elastic, by use the above codes.
   !----
    ...
    if (time_stepping_scheme == 1) then
      ...
      if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
       call update_displacement_newmark_elastic_m2(deltat,deltatover2,deltatsquareover2, &
                                               accel_elastic_m2,veloc_elastic_m2, &
                                               displ_elastic_m2,displ_elastic_old_m2, &
                                               PML_BOUNDARY_CONDITIONS)
       !update also for m1 if compute_appro_Hessian=true
       if(compute_appro_Hessian) then !for approximate Hessian
          call update_displacement_newmark_elastic_m1(deltat,deltatover2,deltatsquareover2, &
                                               accel_elastic_m1,veloc_elastic_m1, &
                                               displ_elastic_m1,displ_elastic_old_m1, &
                                               PML_BOUNDARY_CONDITIONS)
       endif
      endif !CTD_SEM
      ...
      if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
        accel_elastic_m2(:,:) = 0._CUSTOM_REAL !lucas, CTD-SEM
        if(compute_appro_Hessian) then !for approximate Hessian
          accel_elastic_m1(:,:) = 0._CUSTOM_REAL !lucas, CTD-SEM
        endif
      endif!CTD_SEM
    endif
  else
...
!
  ...
  subroutine update_displ_elastic_backward() !lucas, only used for forward reconstruction back in time
  ...
  if (.not. GPU_MODE) then
    ...
    if (time_stepping_scheme == 1) then
      call update_displacement_newmark_elastic(b_deltat,b_deltatover2,b_deltatsquareover2, &
                                               ...
                                               .false.)
      if(CTD_SEM) then !lucas1, CTD-SEM only
      call update_displacement_newmark_elastic_m2(b_deltat,b_deltatover2,b_deltatsquareover2, &
                                               b_accel_elastic_m2,b_veloc_elastic_m2, &
                                               b_displ_elastic_m2,b_displ_elastic_old_m2, &
                                               .false.) ! lucas, can use the above subroutine
      endif

    else
      b_accel_elastic(:,:) = 0._CUSTOM_REAL
      if(CTD_SEM) then !lucas2, CTD-SEM only
...
!
 ...
 !lucas, CTD-SEM
 subroutine update_displacement_newmark_elastic_m2(deltat,deltatover2,deltatsquareover2, &
                                                 accel_elastic_m2,veloc_elastic_m2, &
                                                 displ_elastic_m2,displ_elastic_old_m2, &
                                                 PML_BOUNDARY_CONDITIONS)
  ...
  double precision,intent(in) :: deltat,deltatover2,deltatsquareover2
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_elastic),intent(inout) :: accel_elastic_m2,veloc_elastic_m2, &
                                                                      displ_elastic_m2,displ_elastic_old_m2
 ...
 ! integer :: iglob
  ...
  if (USE_ENFORCE_FIELDS) then
    if (USE_ENFORCE_FIELDS) & 
       call stop_the_code('Updating displacement for USE_ENFORCE_FIELDS for CTD-SEM method not implemented yet') !lucas, CTD-SEM
   ! do iglob = 1,nglob_elastic
   !   if (iglob_is_forced(iglob)) then
   !     if (elastic_iglob_is_forced(iglob)) call enforce_fields(iglob,it) ! TODO TODO TODO
   !   else
  ...
  else ! lucas, CTD-SEM, only changed here
    displ_elastic_m2(:,:) = displ_elastic_m2(:,:) + deltat * veloc_elastic_m2(:,:) + deltatsquareover2 * accel_elastic_m2(:,:)
    veloc_elastic_m2(:,:) = veloc_elastic_m2(:,:) + deltatover2 * accel_elastic_m2(:,:)
    accel_elastic_m2(:,:) = 0._CUSTOM_REAL
  endif

  end subroutine update_displacement_newmark_elastic_m2

 !lucas, CTD-SEM, for approximate Hessian
 subroutine update_displacement_newmark_elastic_m1(deltat,deltatover2,deltatsquareover2, &
                                                 accel_elastic_m1,veloc_elastic_m1, &
                                                 displ_elastic_m1,displ_elastic_old_m1, &
                                                 PML_BOUNDARY_CONDITIONS)
  ...
  double precision,intent(in) :: deltat,deltatover2,deltatsquareover2
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_elastic),intent(inout) :: accel_elastic_m1,veloc_elastic_m1, &
                                                                      displ_elastic_m1,displ_elastic_old_m1
 ...
 ! integer :: iglob
  ...
  if (USE_ENFORCE_FIELDS) then
   ...
   ! enddo
    ...
    displ_elastic_m1(:,:) = displ_elastic_m1(:,:) + deltat * veloc_elastic_m1(:,:) + deltatsquareover2 * accel_elastic_m1(:,:)
    veloc_elastic_m1(:,:) = veloc_elastic_m1(:,:) + deltatover2 * accel_elastic_m1(:,:)
    accel_elastic_m1(:,:) = 0._CUSTOM_REAL
  endif

  end subroutine update_displacement_newmark_elastic_m1
...
!------------lucas, CTD-SEM----------------------------------------------------------------------------
  ...
  subroutine update_veloc_elastic_Newmark_m2()
...
! updates velocity vector (corrector)
  ...
  if (USE_ENFORCE_FIELDS) then
    ...
    veloc_elastic_m2(:,:) = veloc_elastic_m2(:,:) + deltatover2 * accel_elastic_m2(:,:) !lucas, CTD-SEM method
 

  endif

  end subroutine update_veloc_elastic_Newmark_m2

!------------lucas, CTD-SEM----------------------------------------------------------------------------

  subroutine update_veloc_elastic_Newmark_m1()
...
! updates velocity vector (corrector)
  ...
  if (USE_ENFORCE_FIELDS) then
    ...
    veloc_elastic_m1(:,:) = veloc_elastic_m1(:,:) + deltatover2 * accel_elastic_m1(:,:) !lucas, CTD-SEM method
 

  endif

  end subroutine update_veloc_elastic_Newmark_m1
...
! updates backward velocity vector (corrector)
  ...
  subroutine update_veloc_elastic_Newmark_backward_m2() !lucas, CTD-SEM-------
...
! updates backward velocity vector (corrector)
  ...
  if (USE_ENFORCE_FIELDS) then
    ...
    b_veloc_elastic_m2(:,:) = b_veloc_elastic_m2(:,:) + b_deltatover2 * b_accel_elastic_m2(:,:)
  endif


  end subroutine update_veloc_elastic_Newmark_backward_m2
