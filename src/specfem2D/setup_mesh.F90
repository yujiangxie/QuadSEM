...
! creates mesh related properties, local to global mesh numbering and node locations
  ...
  call setup_mesh_external_models() ! lucas: to get vpext(i,j,ispec) etc. lucas, alread implemented for CTD-SEM
...
!
  ...
  subroutine setup_mesh_external_models() ! lucas: to get vpext(i,j,ispec) etc.
...
! external models
 ...
 ! lucas, CTD-SEM
  ...
  allocate(vpext_m2(NGLLX,NGLLZ,nspec_ext), &
           vsext_m2(NGLLX,NGLLZ,nspec_ext), &
           rhoext_m2(NGLLX,NGLLZ,nspec_ext), &
           QKappa_attenuationext_m2(NGLLX,NGLLZ,nspec_ext), &
           Qmu_attenuationext_m2(NGLLX,NGLLZ,nspec_ext),stat=ier)
  if (ier /= 0) call stop_the_code('Error allocating external model arrays (CTD-SEM for m2) for vp vs rho attenuation')
  endif
