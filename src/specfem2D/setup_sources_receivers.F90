...
!========================================================================
  ...
  call setup_sources()!lucas: got: out of the locate_source are: ispec_selected_source,islice_selected_source,iglob_source, and xi_source,gamma_source

  ! reads in stations file and locates receivers
  call setup_receivers()

  ! reads in adjoint sources
  call setup_adjoint_sources() ! lucas: got source_adjoint(irec_local,:,1) and source_adjoint(irec_local,:,2)
...
! compute source array for adjoint source
  ...
  use specfem_par, only: nadj_rec_local,nrec,nrecloc,NSTEP,NPROC,SIMULATION_TYPE,SU_FORMAT, &
                        ...
                        network_name,station_name,source_adjoint, source_adjoint_m2, compute_appro_Hessian !lucas, CTD-SEM
  ...
  if (SIMULATION_TYPE == 2 .or. SIMULATION_TYPE == 3) then
    ...
    allocate(source_adjoint_m2(nrecloc,NSTEP,2),stat=ier)
    if (ier /= 0) stop 'Error allocating array source_adjoint_m2'
    source_adjoint_m2(:,:,:) = 0._CUSTOM_REAL
    endif !------------------------------------
    ...
    if (.not. SU_FORMAT) then
      ...
      irec_local = 0
      do irec = 1, nrec !lucas
        ! compute only adjoint source arrays in the local proc
       if (myrank == islice_selected_rec(irec)) then !lucas
         irec_local = irec_local + 1
         adj_source_file = trim(network_name(irec))//'.'//trim(station_name(irec)) 
         !lucas, alread included read source_adjoint_m2 for approximate Hessian
         call read_adj_source(irec_local,seismotype_adj,adj_source_file) !lucas:in compute_arrays_source.f90, where got source_adjoint(irec_local,:,1) and source_adjoint(irec_local,:,2)
    ...
    if (myrank == 0) then
      ...
      write(IMAIN,*) '  adjoint sources min/max   = ',minval(source_adjoint(:,:,:)),maxval(source_adjoint(:,:,:))
        if(compute_appro_Hessian) then
          write(IMAIN,*) '  adjoint sources min/max for m2   = ',minval(source_adjoint_m2(:,:,:)),maxval(source_adjoint_m2(:,:,:))
        endif
      call flush_IMAIN()
    endif
    ...
    if (myrank == islice_selected_source(i_source)) then
      ...
      if (AXISYM) then
        if (is_on_the_axis(ispec)) then
          call lagrange_any(xi_source(i_source),NGLJ,xiglj,hxis,hpxis)! lucas, out: the last two items, i.e., Lagrange interpolants and their first derivatives 
        ...
        call lagrange_any(xi_source(i_source),NGLLX,xigll,hxis,hpxis) ! lucas. xi_source and gamma_source are coordinate of GLL, 
      endif
...
!! DK DK May 2018: and more generally compared to reality
                sourcearray(1,i,j) = + sin(anglesource(i_source)) * hlagrange
                sourcearray(2,i,j) = - cos(anglesource(i_source)) * hlagrange
                !sourcearray(1,i,j) =  sin(30*3.1419/180.0)*hlagrange
                !sourcearray(2,i,j) =  -cos(30*3.14159/180.0)*hlagrange
...
!! DK DK May 2018: and more generally compared to reality
      ...
      !lucas debug
      print *, '*********########*********setup_sources_receivers.F90'
  ...
  do irec = 1,nrec
    if (AXISYM) then
      if (is_on_the_axis(ispec_selected_rec(irec)) .and. myrank == islice_selected_rec(irec)) then
        call lagrange_any(xi_receiver(irec),NGLJ,xiglj,hxir,hpxir) ! lucas, out: the last two items, i.e., Lagrange interpolants and their first derivatives 
        ...
        call lagrange_any(xi_receiver(irec),NGLLX,xigll,hxir,hpxir) !lucas. xi_receiver and gamma_receiver are coordinate of GLL, 
      endif
    else
    ...
    if (myrank == islice_selected_rec(irec)) then
      ...
      gammar_store_loc(irec_local,:) = hgammar(:)
        ...
        ! lucas, debug
       !print *, 'setup_source_receiver->setup_receiver_interpolation:'
       !print *, '********#######*****xir_store_loc(irec_local,:)=',xir_store_loc(irec_local,:)
       !print *, '********#######*****gammar_store_loc(irec_local,:)=',gammar_store_loc(irec_local,:)
       !print *, '********#######*****hxir(:)=',hxir(:)
       !print *, '********#######*****hgammar(:)=',hgammar(:)
       !print *, 
