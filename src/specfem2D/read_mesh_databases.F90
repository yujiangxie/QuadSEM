...
!========================================================================
  ...
  read(IIN) compute_appro_Hessian

  read(IIN) Full_Hessian_by_Wavefield_Stored

  read(IIN) TOMOGRAPHY_FILE_m2
...
!
  ...
  ! material properties
  call read_mesh_databases_mato() ! lucas:read_materials()
...
! reads in absorbing edges
  ...
  if (anyabs) then
    ...
    if (any_elastic .and. (SAVE_FORWARD .or. SIMULATION_TYPE == 3) .and. STACEY_ABSORBING_CONDITIONS) then
     ...
     ! lucas, CTD-SEM 
      allocate(b_absorb_elastic_left_m2(NDIM,NGLLZ,nspec_left,NSTEP))
      allocate(b_absorb_elastic_right_m2(NDIM,NGLLZ,nspec_right,NSTEP))
      allocate(b_absorb_elastic_bottom_m2(NDIM,NGLLX,nspec_bottom,NSTEP))
      allocate(b_absorb_elastic_top_m2(NDIM,NGLLX,nspec_top,NSTEP))

      allocate(b_absorb_elastic_left_m1(NDIM,NGLLZ,nspec_left,NSTEP))
      allocate(b_absorb_elastic_right_m1(NDIM,NGLLZ,nspec_right,NSTEP))
      allocate(b_absorb_elastic_bottom_m1(NDIM,NGLLX,nspec_bottom,NSTEP))
      allocate(b_absorb_elastic_top_m1(NDIM,NGLLX,nspec_top,NSTEP))
    else
      ...
      ! lucas, CTD-SEM
      allocate(b_absorb_elastic_left_m2(1,1,1,1))
      allocate(b_absorb_elastic_right_m2(1,1,1,1))
      allocate(b_absorb_elastic_bottom_m2(1,1,1,1))
      allocate(b_absorb_elastic_top_m2(1,1,1,1))

      allocate(b_absorb_elastic_left_m1(1,1,1,1))
      allocate(b_absorb_elastic_right_m1(1,1,1,1))
      allocate(b_absorb_elastic_bottom_m1(1,1,1,1))
      allocate(b_absorb_elastic_top_m1(1,1,1,1))
    ...
    ! lucas, CTD-SEM
    if (.not. allocated(b_absorb_elastic_left_m2)) then
      allocate(b_absorb_elastic_left_m2(1,1,1,1))
      allocate(b_absorb_elastic_right_m2(1,1,1,1))
      allocate(b_absorb_elastic_bottom_m2(1,1,1,1))
      allocate(b_absorb_elastic_top_m2(1,1,1,1))
    ...
    if (.not. allocated(b_absorb_elastic_left_m1)) then
      allocate(b_absorb_elastic_left_m1(1,1,1,1))
      allocate(b_absorb_elastic_right_m1(1,1,1,1))
      allocate(b_absorb_elastic_bottom_m1(1,1,1,1))
      allocate(b_absorb_elastic_top_m1(1,1,1,1))
