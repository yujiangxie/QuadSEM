...
! checks simulation stability and outputs timerun infos
  ...
  real(kind=CUSTOM_REAL) :: displnorm_all_m2,displnorm_all_glob_m2 ! lucas, CTD-SEM
  real(kind=CUSTOM_REAL) :: displnorm_all_m1,displnorm_all_glob_m1 ! lucas, CTD-SEM
  ...
  if (.not. GPU_MODE) then ! todo: wavefields are on GPU and not transferred onto CPU yet for the following norm checks
      ...
      if (ELASTIC_SIMULATION) then
        if (any_elastic) then
          ...
          displnorm_all_m2 = maxval(sqrt(displ_elastic_m2(1,:)**2 + displ_elastic_m2(2,:)**2)) !lucas, CTD-SEM
            if(compute_appro_Hessian) then ! for approximate Hessian
            displnorm_all_m1 = maxval(sqrt(displ_elastic_m1(1,:)**2 + displ_elastic_m1(2,:)**2)) !lucas, CTD-SEM
            endif
          endif
        else
          displnorm_all = 0._CUSTOM_REAL
          if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
          displnorm_all_m2 = 0._CUSTOM_REAL  ! lucas, CTD-SEM
            if(compute_appro_Hessian) then ! for approximate Hessian
            displnorm_all_m1 = 0._CUSTOM_REAL  ! lucas, CTD-SEM
            endif
          endif
        endif
        ...
        if (CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
        call max_all_cr(displnorm_all_m2, displnorm_all_glob_m2) !lucas, CTD-SEM
          if(compute_appro_Hessian) then ! for approximate Hessian
            call max_all_cr(displnorm_all_m1, displnorm_all_glob_m1) !lucas, CTD-SEM
          endif
        endif
        if (myrank == 0) &
          write(IMAIN,*) 'Max norm of vector field in solid (elastic) = ', displnorm_all_glob
          if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
          write(IMAIN,*) 'Max norm of vector field in solid (elastic) for m2 = ', displnorm_all_glob_m2
            if(compute_appro_Hessian) then ! for approximate Hessian
            write(IMAIN,*) 'Max norm of vector field in solid (elastic) for m1 = ', displnorm_all_glob_m1
            endif
          endif
        ...
        ! lucas, CTD-SEM-----------------------
        if (CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
          if (displnorm_all_m2 > STABILITY_THRESHOLD .or. displnorm_all_m2 < 0._CUSTOM_REAL .or. &
          ! this trick checks for NaN (Not a Number), which is not even equal to itself
            displ_elastic_m2(1,1) /= displ_elastic_m2(1,1)) &
          call exit_MPI(myrank,'code became unstable and blew up in solid (elastic), m2, CTD-SEM')

          if(compute_appro_Hessian) then ! for approximate Hessian
             if (displnorm_all_m1 > STABILITY_THRESHOLD .or. displnorm_all_m1 < 0._CUSTOM_REAL .or. &
             ! this trick checks for NaN (Not a Number), which is not even equal to itself
             displ_elastic_m1(1,1) /= displ_elastic_m1(1,1)) &
             call exit_MPI(myrank,'code became unstable and blew up in solid (elastic),m1, CTD-SEM')
          endif  
