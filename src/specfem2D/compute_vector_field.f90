...
! and combine with existing velocity vector field in elastic elements
  ...
  subroutine compute_vector_whole_medium_m2(field_acoustic,field_elastic_m2,fields_poroelastic)
  !lucas, from field_elastic to vector_field_element to vector_field_display
...
! and combine with existing velocity vector field in elastic elements
  ...
  use specfem_par_movie, only: vector_field_display_m2 !lucas, CTD-SEM

  implicit none

  real(kind=CUSTOM_REAL), dimension(nglob_acoustic) :: field_acoustic
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_elastic) :: field_elastic_m2 !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_poroelastic) :: fields_poroelastic
  ! vector field in an element
  ...
  do ispec = 1,nspec
    ! computes vector field in this element
    call compute_vector_one_element(field_acoustic,field_elastic_m2,fields_poroelastic,ispec,vector_field_element_m2) !lucas, CTD-SEM
    !lucas, only implemened for elastic
    
    ! stores the result on global nodes
    do j = 1,NGLLZ
      do i = 1,NGLLX
        iglob = ibool(i,j,ispec)
        vector_field_display_m2(:,iglob) = vector_field_element_m2(:,i,j)
      enddo
    enddo

  enddo

  end subroutine compute_vector_whole_medium_m2


  subroutine compute_vector_whole_medium_m1(field_acoustic,field_elastic_m1,fields_poroelastic) ! lucas field_elastic_m1=displ_elastic_m1
  !lucas, from field_elastic to vector_field_element to vector_field_display
...
! and combine with existing velocity vector field in elastic elements
  ...
  use specfem_par_movie, only: vector_field_display_m1 !lucas, CTD-SEM

  implicit none

  real(kind=CUSTOM_REAL), dimension(nglob_acoustic) :: field_acoustic
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_elastic) :: field_elastic_m1 !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob_poroelastic) :: fields_poroelastic
  ! vector field in an element
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLLX,NGLLZ) :: vector_field_element_m1 !lucas, CTD-SEM
  ...
  do ispec = 1,nspec
    ! computes vector field in this element
    call compute_vector_one_element(field_acoustic,field_elastic_m1,fields_poroelastic,ispec,vector_field_element_m1) !lucas, CTD-SEM
    !lucas, only implemened for elastic
    ...
    do j = 1,NGLLZ
      do i = 1,NGLLX
        iglob = ibool(i,j,ispec)
        vector_field_display_m1(:,iglob) = vector_field_element_m1(:,i,j)
      enddo
    enddo

  enddo

  end subroutine compute_vector_whole_medium_m1
