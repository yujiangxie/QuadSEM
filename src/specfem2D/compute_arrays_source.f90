...
! reads in adjoint source file
  ...
  use specfem_par, only: myrank, NSTEP, DT, P_SV, source_adjoint,source_adjoint_m2, f0_source, compute_appro_Hessian !lucas, CTD-SEM
  ...
  double precision :: junk,junk_m2
  double precision :: f1, f2
  real(kind=CUSTOM_REAL), dimension(NSTEP) :: temp
  real(kind=CUSTOM_REAL), dimension(NSTEP,3) :: adj_src_s, adj_src_s_m2 !lucas

  character(len=3) :: comp(3)
  character(len=MAX_STRING_LEN) :: filename,filename_m2 !lucas
  ...
  case (1,2,3)
    ...
    !lucas, open adj(m+dm) for approximate Hessian----------------------------
    ...
    do icomp = 1,3
      ...
      ! reads in ascii adjoint source files **.adj
      filename_m2 = 'SEM/'//trim(adj_source_file) // '.'// comp(icomp) // '.adj_m2' !lucas, adjoint source file name saved in SEM/
      open(unit = IIN+100, file = trim(filename_m2), iostat = ier)
      if (ier /= 0) then
        print *,'Error: could not find adjoint source file (m2) ',trim(filename_m2)
        print *,'Please check if adjoint file for m2 exists...'
        call exit_MPI(myrank,'file '//trim(filename_m2)//' does not exist')
      endif

      do itime = 1, NSTEP
        read(IIN+100,*) junk_m2, adj_src_s_m2(itime,icomp)
      enddo
      close(IIN+100)
    enddo
    endif ! compute_appro_Hessian---------------------------------------
  ...
  if (P_SV) then
    ! P_SV-case
    source_adjoint(irec_local,:,1) = real(adj_src_s(:,1),kind=CUSTOM_REAL) ! lucas, x component
    source_adjoint(irec_local,:,2) = real(adj_src_s(:,3),kind=CUSTOM_REAL) ! lucas, z component
  else
    ! SH-case
  ...
  if (P_SV) then
    ! P_SV-case
    source_adjoint_m2(irec_local,:,1) = real(adj_src_s_m2(:,1),kind=CUSTOM_REAL) ! lucas, x component
    source_adjoint_m2(irec_local,:,2) = real(adj_src_s_m2(:,3),kind=CUSTOM_REAL) ! lucas, z component
  else
    ! SH-case
    source_adjoint_m2(irec_local,:,1) = real(adj_src_s_m2(:,2),kind=CUSTOM_REAL)
  endif
  endif! compute_appro_Hessian-----------------------------
