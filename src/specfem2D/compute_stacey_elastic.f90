...
subroutine compute_stacey_elastic_m2(accel_elastic_m2,veloc_elastic_m2)
! used for CTD-SEM method only, Yujiang Lucas Xie
! absorbing boundaries
!
! Clayton-Engquist condition if elastic
  ...
  use specfem_par, only: nglob,nelemabs,it,any_elastic, &
                         assign_external_model,ibool,numabs,ispec_is_elastic, &
                         codeabs,codeabs_corner,xix,xiz,gammax,gammaz,jacobian, &
                         vpext_m2,vsext_m2,rhoext_m2,wxgll,wzgll,P_SV, & ! lucas, CTD-SEM
                         SIMULATION_TYPE,SAVE_FORWARD, &
 ...
 ! use specfem_par, only: x_source,z_source,f0_source
  ...
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob),intent(in) :: veloc_elastic_m2 !lucas, CTD-SEM

  ! local parameters
  integer :: ispecabs,ispec,i,j,iglob
  real(kind=CUSTOM_REAL) :: weight,xxi,zxi,xgamma,zgamma,jacobian1D
  real(kind=CUSTOM_REAL) :: nx,nz,vx_m2,vy_m2,vz_m2,vn_m2,rho_vp_m2,rho_vs_m2,tx_m2,ty_m2,tz_m2 !lucas, CTD-SEM

  ! material properties of the elastic medium
  real(kind=CUSTOM_REAL) :: cpl_m2,csl_m2,rhol_m2 ! lucas, CTD-SEM

  ! for analytical initial plane wave for Bielak's conditions
  double precision :: veloc_horiz,veloc_vert,traction_x_t0,traction_z_t0
  integer :: count_left,count_right,count_bottom
  ...
  do ispecabs = 1,nelemabs
    ...
    if (codeabs(IEDGE4,ispecabs)) then
      ...
      do j = 1,NGLLZ
        ...
        ! for analytical initial plane wave for Bielak's conditions
        ! left or right edge, horizontal normal vector
       ! lucas, so far, CTD-SEM is not implemented for Bielak_conditions and plane wave,-----------
       ! if (add_Bielak_conditions_left .and. initialfield) then
       !   if (.not. over_critical_angle) then
       !     call compute_Bielak_conditions(coord,iglob,nglob,it,deltat,dxUx,dxUz,dzUx,dzUz,veloc_horiz,veloc_vert, &
       !                 x_source(1), z_source(1), A_plane, B_plane, C_plane, anglesource(1), anglesource_refl, &
       ...
       ! endif
        ...
        rho_vp_m2 = rhol_m2*cpl_m2
        rho_vs_m2 = rhol_m2*csl_m2
        ...
        if (P_SV) then !lucas 2, CTD-SEM
          ! P_SV case
          vx_m2 = veloc_elastic_m2(1,iglob) - veloc_horiz
          vz_m2 = veloc_elastic_m2(2,iglob) - veloc_vert
          vn_m2 = nx*vx_m2 + nz*vz_m2

          tx_m2 = rho_vp_m2*vn_m2*nx + rho_vs_m2*(vx_m2-vn_m2*nx)
          tz_m2 = rho_vp_m2*vn_m2*nz + rho_vs_m2*(vz_m2-vn_m2*nz)

        else
          ! SH case
          vy_m2 = veloc_elastic_m2(1,iglob)
          ty_m2 = rho_vs_m2*vy_m2
        endif

        weight = jacobian1D * wzgll(j)

        if (P_SV) then
          ! P_SV case, lucas 3, CTD-SEM
          accel_elastic_m2(1,iglob) = accel_elastic_m2(1,iglob) - (tx_m2 + traction_x_t0)*weight
          accel_elastic_m2(2,iglob) = accel_elastic_m2(2,iglob) - (tz_m2 + traction_z_t0)*weight
        else
          ! SH case
          accel_elastic_m2(1,iglob) = accel_elastic_m2(1,iglob) - ty_m2*weight
        endif

        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then
            ! P-SV waves, lucas 4, CTD-SEM
            b_absorb_elastic_left_m2(1,j,ib_left(ispecabs),it) = (tx_m2 + traction_x_t0)*weight
            b_absorb_elastic_left_m2(2,j,ib_left(ispecabs),it) = (tz_m2 + traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_left_m2(1,j,ib_left(ispecabs),it) = ty_m2*weight
          endif
        endif
      enddo
    endif  !  end of left absorbing boundary
    ...
    if (codeabs(IEDGE2,ispecabs)) then
      ...
      do j = 1,NGLLZ
       ...
       ! endif
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then ! lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_right_m2(1,j,ib_right(ispecabs),it) = (tx_m2 - traction_x_t0)*weight
            b_absorb_elastic_right_m2(2,j,ib_right(ispecabs),it) = (tz_m2 - traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_right_m2(1,j,ib_right(ispecabs),it) = ty_m2*weight
          endif
        endif
      enddo
    endif  !  end of right absorbing boundary
...
! thus subtract nothing i.e. zero in that case
        if ((codeabs_corner(1,ispecabs) .and. i == 1) .or. (codeabs_corner(2,ispecabs) .and. i == NGLLX)) then
          tx_m2 = 0._CUSTOM_REAL ! lucas, CTD-SEM
          ty_m2 = 0._CUSTOM_REAL ! lucas, CTD-SEM
          tz_m2 = 0._CUSTOM_REAL !lucas, CTD-SEM
        endif
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then ! lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_bottom_m2(1,i,ib_bottom(ispecabs),it) = (tx_m2 + traction_x_t0)*weight
            b_absorb_elastic_bottom_m2(2,i,ib_bottom(ispecabs),it) = (tz_m2 + traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_bottom_m2(1,i,ib_bottom(ispecabs),it) = ty_m2*weight
          endif
        endif
      enddo
    endif  !  end of bottom absorbing boundary
...
! thus subtract nothing i.e. zero in that case
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then !lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_top_m2(1,i,ib_top(ispecabs),it) = (tx_m2 - traction_x_t0)*weight
            b_absorb_elastic_top_m2(2,i,ib_top(ispecabs),it) = (tz_m2 - traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_top_m2(1,i,ib_top(ispecabs),it) = ty_m2*weight
          endif
        endif
      enddo
    endif  !  end of top absorbing boundary
  ...
  end subroutine compute_stacey_elastic_m2

! lucas, CTD-SEM---------m1----------------------------------------------

subroutine compute_stacey_elastic_m1(accel_elastic_m1,veloc_elastic_m1)
! used for CTD-SEM method only, Yujiang Lucas Xie
! absorbing boundaries
!
! Clayton-Engquist condition if elastic
  ...
  use specfem_par, only: nglob,nelemabs,it,any_elastic, &
                         assign_external_model,ibool,numabs,ispec_is_elastic, &
                         codeabs,codeabs_corner,xix,xiz,gammax,gammaz,jacobian, &
                         vpext,vsext,rhoext,wxgll,wzgll,P_SV, & ! lucas, CTD-SEM
                         SIMULATION_TYPE,SAVE_FORWARD, &
                         b_absorb_elastic_left_m1,b_absorb_elastic_right_m1, & ! lucas, CTD-SEM
                         b_absorb_elastic_bottom_m1,b_absorb_elastic_top_m1, & ! lucas, CTD-SEM
                         ib_left,ib_right,ib_bottom,ib_top, &
                         STACEY_ABSORBING_CONDITIONS
 ...
 ! use specfem_par, only: x_source,z_source,f0_source
  ...
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob),intent(in) :: veloc_elastic_m1 !lucas, CTD-SEM

  ! local parameters
  integer :: ispecabs,ispec,i,j,iglob
  real(kind=CUSTOM_REAL) :: weight,xxi,zxi,xgamma,zgamma,jacobian1D
  real(kind=CUSTOM_REAL) :: nx,nz,vx_m1,vy_m1,vz_m1,vn_m1,rho_vp_m1,rho_vs_m1,tx_m1,ty_m1,tz_m1 !lucas, CTD-SEM

  ! material properties of the elastic medium
  real(kind=CUSTOM_REAL) :: cpl_m1,csl_m1,rhol_m1 ! lucas, CTD-SEM
    ...
    if (codeabs(IEDGE4,ispecabs)) then
      ...
      do j = 1,NGLLZ
       ...
       ! endif
        ...
        rho_vp_m1 = rhol_m1*cpl_m1
        rho_vs_m1 = rhol_m1*csl_m1
        ...
        if (P_SV) then !lucas 2, CTD-SEM
          ! P_SV case
          vx_m1 = veloc_elastic_m1(1,iglob) - veloc_horiz
          vz_m1 = veloc_elastic_m1(2,iglob) - veloc_vert
          vn_m1 = nx*vx_m1 + nz*vz_m1

          tx_m1 = rho_vp_m1*vn_m1*nx + rho_vs_m1*(vx_m1-vn_m1*nx)
          tz_m1 = rho_vp_m1*vn_m1*nz + rho_vs_m1*(vz_m1-vn_m1*nz)

        else
          ! SH case
          vy_m1 = veloc_elastic_m1(1,iglob)
          ty_m1 = rho_vs_m1*vy_m1
        endif
        ...
        if (P_SV) then
          ! P_SV case, lucas 3, CTD-SEM
          accel_elastic_m1(1,iglob) = accel_elastic_m1(1,iglob) - (tx_m1 + traction_x_t0)*weight
          accel_elastic_m1(2,iglob) = accel_elastic_m1(2,iglob) - (tz_m1 + traction_z_t0)*weight
        else
          ! SH case
          accel_elastic_m1(1,iglob) = accel_elastic_m1(1,iglob) - ty_m1*weight
        endif

        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then
            ! P-SV waves, lucas 4, CTD-SEM
            b_absorb_elastic_left_m1(1,j,ib_left(ispecabs),it) = (tx_m1 + traction_x_t0)*weight
            b_absorb_elastic_left_m1(2,j,ib_left(ispecabs),it) = (tz_m1 + traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_left_m1(1,j,ib_left(ispecabs),it) = ty_m1*weight
          endif
        endif
      enddo
    endif  !  end of left absorbing boundary
      ...
      do j = 1,NGLLZ
       ...
       ! endif
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then ! lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_right_m1(1,j,ib_right(ispecabs),it) = (tx_m1 - traction_x_t0)*weight
            b_absorb_elastic_right_m1(2,j,ib_right(ispecabs),it) = (tz_m1 - traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_right_m1(1,j,ib_right(ispecabs),it) = ty_m1*weight
          endif
        endif
      enddo
    endif  !  end of right absorbing boundary
...
! thus subtract nothing i.e. zero in that case
        if ((codeabs_corner(1,ispecabs) .and. i == 1) .or. (codeabs_corner(2,ispecabs) .and. i == NGLLX)) then
          tx_m1 = 0._CUSTOM_REAL ! lucas, CTD-SEM
          ty_m1 = 0._CUSTOM_REAL ! lucas, CTD-SEM
          tz_m1 = 0._CUSTOM_REAL !lucas, CTD-SEM
        endif
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then ! lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_bottom_m1(1,i,ib_bottom(ispecabs),it) = (tx_m1 + traction_x_t0)*weight
            b_absorb_elastic_bottom_m1(2,i,ib_bottom(ispecabs),it) = (tz_m1 + traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_bottom_m1(1,i,ib_bottom(ispecabs),it) = ty_m1*weight
          endif
        endif
      enddo
    endif  !  end of bottom absorbing boundary
...
! thus subtract nothing i.e. zero in that case
        ...
        if (SAVE_FORWARD .and. SIMULATION_TYPE == 1) then
          if (P_SV) then !lucas 4, CTD-SEM
            ! P-SV waves
            b_absorb_elastic_top_m1(1,i,ib_top(ispecabs),it) = (tx_m1 - traction_x_t0)*weight
            b_absorb_elastic_top_m1(2,i,ib_top(ispecabs),it) = (tz_m1 - traction_z_t0)*weight
          else
            ! SH (membrane) waves
            b_absorb_elastic_top_m1(1,i,ib_top(ispecabs),it) = ty_m1*weight
          endif
        endif
      enddo
    endif  !  end of top absorbing boundary
  ...
  end subroutine compute_stacey_elastic_m1
...
!lucas-------------------CTD-SEM---------------------------------------------------------------------------------
  ...
  subroutine compute_stacey_elastic_backward_m2(b_accel_elastic_m2)
...
! Clayton-Engquist condition if elastic
  ...
  real(kind=CUSTOM_REAL), dimension(NDIM,nglob),intent(inout) :: b_accel_elastic_m2 !lucas, CTD-SEM
  ...
  do ispecabs = 1,nelemabs
    ...
    if (codeabs(IEDGE4,ispecabs)) then
      ...
      do j = 1,NGLLZ
        ! Clayton-Engquist condition if elastic
        iglob = ibool(i,j,ispec)
        if (P_SV) then !lucas1, CTD-SEM
          ! P-SV waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_left_m2(1,j,ib_left(ispecabs),it_tmp)
          b_accel_elastic_m2(2,iglob) = b_accel_elastic_m2(2,iglob) - b_absorb_elastic_left_m2(2,j,ib_left(ispecabs),it_tmp)
        else
          ! SH (membrane) waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_left_m2(1,j,ib_left(ispecabs),it_tmp)
        endif
    ...
    if (codeabs(IEDGE2,ispecabs)) then
      ...
      do j = 1,NGLLZ
        iglob = ibool(i,j,ispec)
        if (P_SV) then !lucas2, CTD-SEM
          ! P-SV waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_right_m2(1,j,ib_right(ispecabs),it_tmp)
          b_accel_elastic_m2(2,iglob) = b_accel_elastic_m2(2,iglob) - b_absorb_elastic_right_m2(2,j,ib_right(ispecabs),it_tmp)
        else
          ! SH (membrane) waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_right_m2(1,j,ib_right(ispecabs),it_tmp)
        endif
    ...
    if (codeabs(IEDGE1,ispecabs)) then
      ...
      do i = 1,NGLLX
        ...
        if (P_SV) then !lucas3, CTD-SEM
          ! P-SV waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_bottom_m2(1,i,ib_bottom(ispecabs),it_tmp)
          b_accel_elastic_m2(2,iglob) = b_accel_elastic_m2(2,iglob) - b_absorb_elastic_bottom_m2(2,i,ib_bottom(ispecabs),it_tmp)
        else
          ! SH (membrane) waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_bottom_m2(1,i,ib_bottom(ispecabs),it_tmp)
        endif
    ...
    if (codeabs(IEDGE3,ispecabs)) then
      ...
      do i = 1,NGLLX
        iglob = ibool(i,j,ispec)
        if (P_SV) then !lucas4, CTD-SEM
          ! P-SV waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_top_m2(1,i,ib_top(ispecabs),it_tmp)
          b_accel_elastic_m2(2,iglob) = b_accel_elastic_m2(2,iglob) - b_absorb_elastic_top_m2(2,i,ib_top(ispecabs),it_tmp)
        else
          ! SH (membrane) waves
          b_accel_elastic_m2(1,iglob) = b_accel_elastic_m2(1,iglob) - b_absorb_elastic_top_m2(1,i,ib_top(ispecabs),it_tmp)
        endif
  ...
  end subroutine compute_stacey_elastic_backward_m2
