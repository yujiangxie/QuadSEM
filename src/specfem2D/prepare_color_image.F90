...
! stores P-velocity model in image_color_vp_display
  ...
  use specfem_par, only: nglob,nspec,ispec_is_elastic,ispec_is_poroelastic,ibool,kmato,compute_appro_Hessian, & !lucas, CTD-SEM
    density,poroelastcoef,NPROC,myrank,assign_external_model,vpext, vpext_m2, CTD_SEM, Full_Hessian_by_Wavefield_Stored !lucas, CTD-SEM

  use specfem_par_movie, only: image_color_vp_display,image_color_vp_display_m2,image_color_vp_display_m1, &
     iglob_image_color, NX_IMAGE_color,NZ_IMAGE_color,nb_pixel_loc,num_pixel_loc,DRAW_WATER_IN_BLUE

  implicit none

  ! local parameters
  double precision :: vp_of_the_model
  double precision :: vp_of_the_model_m2,vp_of_the_model_m1 !lucas, CTD-SEM, used for acoustic in the future
  double precision, dimension(:), allocatable :: vp_display
  double precision, dimension(:), allocatable :: vp_display_m2,vp_display_m1 !lucas, CTD-SEM
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas1, CTD-SEM
   allocate(vp_display_m2(nglob))
   if(compute_appro_Hessian) then
     allocate(vp_display_m1(nglob))
   endif
  endif 
        ...
        do i = 1,NGLLX
          ...
          if (assign_external_model) then
            ...
            if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas2, CTD-SEM
              vp_display_m2(ibool(i,j,ispec)) = vpext_m2(i,j,ispec)
              if(compute_appro_Hessian) then !---
                vp_display_m1(ibool(i,j,ispec)) = vpext(i,j,ispec) ! lucas vpext for m1
              endif !---
            endif
          else
        ...
        do i = 1,NGLLX
          ...
          if (assign_external_model) then
            ...
            if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas3, CTD-SEM
              vp_of_the_model_m2 = vpext_m2(i,j,ispec)
              if(compute_appro_Hessian) then
                vp_of_the_model_m1 = vpext(i,j,ispec) ! lucas vpext for m1
              endif
            endif
...
! thus check that Vp is the typical Vp of water
          ...
          if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas4, CTD-SEM
            if (abs(vp_of_the_model_m2 - 1480.d0) <= 50.d0) vp_display_m2(ibool(i,j,ispec)) = -1
            if(compute_appro_Hessian) then !---
              if (abs(vp_of_the_model_m1 - 1480.d0) <= 50.d0) vp_display_m1(ibool(i,j,ispec)) = -1
            endif !---
          endif
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas5, CTD-SEM
    image_color_vp_display_m2(:,:) = 0.d0
    if(compute_appro_Hessian) then
      image_color_vp_display_m1(:,:) = 0.d0
    endif
  endif
...
! avoid edge effects
    ...
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas6, CTD-SEM
     if (iglob_image_color(i,j) /= -1) image_color_vp_display_m2(i,j) = vp_display_m2(iglob_image_color(i,j))
     if(compute_appro_Hessian) then !---
       if (iglob_image_color(i,j) /= -1) image_color_vp_display_m1(i,j) = vp_display_m1(iglob_image_color(i,j))
     endif !---
    endif
  enddo
...
! assembling array image_color_vp_display on process zero for color output
  ...
  if (myrank == 0) then
     ...
     if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas7, CTD-SEM
        allocate(data_pixel_recv_m2(maxval(tmp_nb_pixel_per_proc(:))))
        if(compute_appro_Hessian) then !---
         allocate(data_pixel_recv_m1(maxval(tmp_nb_pixel_per_proc(:))))
        endif !---
     endif
  endif
    ...
    if (myrank == 0) then
      do iproc = 1, NPROC-1
        ...
        if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas9, CTD-SEM
         call recv_dp(data_pixel_recv_m2(1),tmp_nb_pixel_per_proc(iproc), iproc, 430)
         if(compute_appro_Hessian) then !---
          call recv_dp(data_pixel_recv_m1(1),tmp_nb_pixel_per_proc(iproc), iproc, 430+100)
         endif !---
        endif 
...
! avoid edge effects
          ...
          if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas10, CTD-SEM
            image_color_vp_display_m2(i,j) = data_pixel_recv_m2(k)
            if(compute_appro_Hessian) then !---
             image_color_vp_display_m1(i,j) = data_pixel_recv_m1(k)
            endif !---
          endif
...
! avoid edge effects
        ...
        if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas11, CTD-SEM
          if (iglob_image_color(i,j) /= -1) data_pixel_send_m2(k) = vp_display_m2(iglob_image_color(i,j))
          if(compute_appro_Hessian) then !---
            if (iglob_image_color(i,j) /= -1) data_pixel_send_m1(k) = vp_display_m1(iglob_image_color(i,j))
          endif !---
        endif
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas13, CTD-SEM
    deallocate(data_pixel_send_m2)
    if(compute_appro_Hessian) then !---
      deallocate(data_pixel_send_m1)
    endif !---
  endif
  if (myrank == 0) then
    deallocate(num_pixel_recv)
    deallocate(data_pixel_recv)
    if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas14, CTD-SEM
      deallocate(data_pixel_recv_m2)
      if(compute_appro_Hessian) then
        deallocate(data_pixel_recv_m1)
      endif 
    endif
  endif
#else
