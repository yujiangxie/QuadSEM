...
! ----------------------------------------------------------------------------------------
  ...
  ! models dimensions, !lucas, CTD-SEM
  double precision  :: END_X, END_Z, END_X_m2, END_Z_m2
  double precision :: ORIGIN_X, ORIGIN_Z, ORIGIN_X_m2, ORIGIN_Z_m2
  double precision :: SPACING_X, SPACING_Z, SPACING_X_m2, SPACING_Z_m2

  ! models parameter records
  double precision, dimension(:), allocatable :: x_tomo,z_tomo,x_tomo_m2,z_tomo_m2 !lucas, CTD-SEM
...
!
  ...
  subroutine define_external_model_from_tomo_file() !lucas, to get vpext(i,j,ispec), etc.
...
! ----------------------------------------------------------------------------------------
  ...
  use specfem_par, only: tomo_material,coord,nspec,ibool,kmato,rhoext,vpext,vsext, &
                       rhoext_m2,vpext_m2,vsext_m2, & ! lucas, CTD-SEM
  ...
  if (myrank == 0) then
    ...
    write(IMAIN,*) '  tomographic model file for m2 from:', trim(TOMOGRAPHY_FILE_m2)
    endif
    call flush_IMAIN()
  endif

  ! Read external tomo file TOMOGRAPHY_FILE
  call read_tomo_file() !lucas, to get vp_tomo(5,5), etc.
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
  call read_tomo_file_m2() !lucas, CTD-SEM, to get vp_tomo_m2(5,5), etc.
  endif
  ...
  do ispec = 1,nspec
    do j = 1,NGLLZ
      do i = 1,NGLLX
        ...
        if (kmato(ispec) == tomo_material) then  ! lucas, in my case, tomo_material =1, only one line is used in the Par_file
...
! ----------------------------------------------------------------------------------------
 ...
 !!-------------------------------------------CTD-SEM for read m2----------------------------
  ...
  if (ier /= 0) then
    print *,'Error: could not open tomography file: ',trim(TOMOGRAPHY_FILE_m2)
    print *,'Please check your settings in Par_file ...'
    call exit_MPI(myrank,'Error reading tomography file for m2 in CTD-SEM')
  endif
  ...
  ! format: #origin_x #origin_z #end_x #end_z
  call tomo_read_next_line(IIN,string_read)
  read(string_read,*) ORIGIN_X_m2, ORIGIN_Z_m2, END_X_m2, END_Z_m2
  ...
  ! --------------------------------------------------------------------------------------
  call tomo_read_next_line(IIN,string_read)
  read(string_read,*) SPACING_X_m2, SPACING_Z_m2
  ...
  ! allocate models parameter records
  allocate(x_tomography_m2(nrecord_m2),z_tomography_m2(nrecord_m2),vp_tomography_m2(nrecord_m2),vs_tomography_m2(nrecord_m2), &
           rho_tomography_m2(nrecord_m2),stat=ier)
  ...
  z_tomo_m2 = z_tomography_m2(::NX_m2)
  ...
  if (irecord /= nrecord_m2 .and. myrank == 0) then
     print *, 'Error: ',trim('./DATA/CTD_SEM_m2.txt'),' has invalid number of records' !lucas, CTD-SEM
     print *, '     number of grid points specified (= NX_m2*NZ_m2):',nrecord_m2
     print *, '     number of file lines for grid points        :',irecord
     call stop_the_code('Error in tomography data file for the grid points definition')
  endif

  if (myrank == 0) then
    write(IMAIN,*) '     Number of grid points = NX_m2*NZ_m2:',nrecord_m2
    write(IMAIN,*)
    call flush_IMAIN()
  endif
