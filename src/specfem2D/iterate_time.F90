...
! *********************************************************
  ...
  do it = 1,NSTEP
    ...
    if (mod(it,NSTEP_BETWEEN_OUTPUT_INFO) == 0 .or. it == 5 .or. it == NSTEP) then
      call check_stability()!lucas, include m2, and m1 (for approximate Hessian)
    endif
    
    do i_stage = 1, stage_time_scheme
      ! updates wavefields using Newmark time scheme
      ! forward wavefield (acoustic/elastic/poroelastic)
      call update_displ_Newmark() !lucas, include m2, m1 (for approximate Hessian)
    ...
    if (SIMULATION_TYPE == 3 .and. it == 1 .and. .not. NO_BACKWARD_RECONSTRUCTION) then 
      call read_forward_arrays() !lucas, inlude CTD-SEM, 
    endif

    if (NO_BACKWARD_RECONSTRUCTION .and. (SAVE_FORWARD .or. SIMULATION_TYPE == 3) ) then
      call manage_no_backward_reconstruction_io() ! luas, no CTD-SEM here
    endif
...
!    endif
 ...
 !   endif
    ...
    !lucas, kernels are summed for all time steps in this if. For a specific time, we can change here and also have to change the if(NSTEP - it = ...) condition in compute_kernels.f90.
...
subroutine manage_no_backward_reconstruction_io()
  ...
  if (SAVE_FORWARD) then
    ...
    if (mod(it,NSTEP_BETWEEN_COMPUTE_KERNELS) == 0 .or. it==1 .or. it == NSTEP ) then  ! lucas changed from (NSTEP-it+1) to it and added it==1, that could be 1, 10, 20 if delta=10
     !lucas------
         !if(myrank==islice_selected_rec(1)) then
         !  iglob_source_lucas=ibool(2,2,ispec_selected_rec(1)) ! lucas, iglobe_source indiates the receiver
         !  if(mod(it,500)==0) then 
         !   write(6,*) 'ispec_selected_source(1) = ',ispec_selected_rec(1)
         !   write(6,*) 'iglob_source = ',iglob_source_lucas
         !   write(6,*) 'in save: displ_elastic(2,iglob_source)',displ_elastic(2,iglob_source_lucas)
         !  endif
         !endif
     !lucas------
  ...
  if (SIMULATION_TYPE == 3 .and. (.not.SAVE_FORWARD)) then  ! lucas, if save_forward =true, we close this call. !!!!!!
     ...
     if (mod(it,NSTEP_BETWEEN_COMPUTE_KERNELS) == 0 .or. it == 1 .or. it == NSTEP) then
         ...
         !   write(6,*) 'ispec_selected_source(1) = ',ispec_selected_rec(1)
         !   write(6,*) 'iglob_source = ',iglob_source_lucas
         !   write(6,*) 'in read:disp_elastic(2,iglob_source) = ',displ_elastic(2,iglob_source_lucas)
         !  endif
         !endif
     !lucas---
