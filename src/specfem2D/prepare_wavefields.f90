...
!========================================================================
  ...
  integer :: ier
  integer :: nstep_lucas ! lucas
 ! logical :: for_test_m2 !lucas,
 !            for_test_m2=.false.
   nstep_lucas=7200 ! lucas:should changed here
 ...
 !-------------------------------------------------------------------------------------------------------------------------------------
 !lucas: for the volume wave field with time
 ! allocate(displ_elastic_lucas(nstep_lucas,NDIM,nglob_elastic), &                   ! lucas: added the first dimensional for nstep 
 !          veloc_elastic_lucas(nstep_lucas,NDIM,nglob_elastic), &
 !          accel_elastic_lucas(nstep_lucas,NDIM,nglob_elastic),stat=ier)
 ! if (ier /= 0) call stop_the_code('Error allocating elastic wavefield arrays')
 ! PML
 !allocate(displ_elastic_old_lucas(nstep_lucas,NDIM,nglob_elastic),stat=ier) ! lucas: added the first dimensional for nstep
 !--------------------------------------------------------------------------------------------------------------------------------------
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
    allocate(displ_elastic_m2(NDIM,nglob_elastic), &                  
           veloc_elastic_m2(NDIM,nglob_elastic), &
           accel_elastic_m2(NDIM,nglob_elastic),stat=ier)
    if (ier /= 0) call stop_the_code('Error allocating elastic wavefield arrays, CTD-SEM, for m2')
    ! PML
    allocate(displ_elastic_old_m2(NDIM,nglob_elastic),stat=ier)

    if(compute_appro_Hessian) then!--------------- for computing approximate Hessian
      allocate(displ_elastic_m1(NDIM,nglob_elastic), &                  
               veloc_elastic_m1(NDIM,nglob_elastic), &
               accel_elastic_m1(NDIM,nglob_elastic),stat=ier)
      if (ier /= 0) call stop_the_code('Error allocating elastic wavefield arrays, CTD-SEM, for m1,approx Hessian')
      ! PML
      allocate(displ_elastic_old_m1(NDIM,nglob_elastic),stat=ier)
    endif !---------------------------
  endif 
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
  allocate(rmass_inverse_elastic_m2(NDIM,nglob_elastic),stat=ier) !lucas, CTD-SEM
  if (ier /= 0) call stop_the_code('Error allocating elastic mass matrix array for m2 in CTD-SEM')
  endif
  ...
  !---------------------------------------------------------------------------------------------------------------
  !lucas: for the volume wave field with time
  !allocate(b_displ_elastic_lucas(nstep_lucas,NDIM,nglob_elastic_b), &   ! lucas, added the first dimensional for nstep
  !         b_veloc_elastic_lucas(nstep_lucas,NDIM,nglob_elastic_b), &
  !         b_accel_elastic_lucas(nstep_lucas,NDIM,nglob_elastic_b),stat=ier)
  !if (ier /= 0) call stop_the_code('Error allocating elastic backward wavefield arrays')

  !allocate(b_displ_elastic_old_lucas(nstep_lucas,NDIM,nglob_elastic_b)) ! lucas, added the first dimensional for nstep
  !---------------------------------------------------------------------------------------------------------------
  ...
  !lucas, CTD-SEM
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
  allocate(b_displ_elastic_m2(NDIM,nglob_elastic_b), &   
           b_veloc_elastic_m2(NDIM,nglob_elastic_b), &
           b_accel_elastic_m2(NDIM,nglob_elastic_b),stat=ier)
  if (ier /= 0) call stop_the_code('Error allocating elastic backward wavefield arrays, CTD-SEM, for m2')
  
  allocate(b_displ_elastic_old_m2(NDIM,nglob_elastic_b)) 
  endif
  ...
  !allocate(rho_k_m2(nglob_elastic_b))
  !allocate(mu_k_m2(nglob_elastic_b))
  !allocate(kappa_k_m2(nglob_elastic_b))
  !for Ha
  allocate(rho_k_Ha(nglob_elastic_b))
  allocate(mu_k_Ha(nglob_elastic_b))
  allocate(kappa_k_Ha(nglob_elastic_b))
  !for Hbm
  allocate(rho_k_Hbm(nglob_elastic_b))
  allocate(mu_k_Hbm(nglob_elastic_b))
  allocate(kappa_k_Hbm(nglob_elastic_b))
  !for Hbs
  if(compute_appro_Hessian) then
   allocate(rho_k_Hbs(nglob_elastic_b))
   allocate(mu_k_Hbs(nglob_elastic_b))
   allocate(kappa_k_Hbs(nglob_elastic_b))
  endif
  !for Hc, the original verison is for Hc
  !allocate(rho_k(nglob_elastic_b))
  !allocate(mu_k(nglob_elastic_b))
  !allocate(kappa_k(nglob_elastic_b))
  endif
  ...
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then !lucas, CTD-SEM----
  allocate(rho_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(mu_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(kappa_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(rhop_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(alpha_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(beta_kl_Ha(NGLLX,NGLLZ,nspec_elastic_b))
  !-------Hbm
  allocate(rho_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(mu_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(kappa_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(rhop_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(alpha_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(beta_kl_Hbm(NGLLX,NGLLZ,nspec_elastic_b))
  if(compute_appro_Hessian) then !Hbs
   allocate(rho_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
   allocate(mu_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
   allocate(kappa_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
   allocate(rhop_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
   allocate(alpha_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
   allocate(beta_kl_Hbs(NGLLX,NGLLZ,nspec_elastic_b))
  endif
  !-------
  allocate(rho_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(mu_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(kappa_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(rhop_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(alpha_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(beta_kl_Hc(NGLLX,NGLLZ,nspec_elastic_b))
  !-------
  allocate(rho_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(mu_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(kappa_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(rhop_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(alpha_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))
  allocate(beta_kl_Habc(NGLLX,NGLLZ,nspec_elastic_b))

  endif !---end CTD-SEM

 ! if(for_test_m2) then!------------------------lucas
 ! allocate(rho_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! allocate(mu_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! allocate(kappa_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! allocate(rhop_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! allocate(alpha_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! allocate(beta_kl_m2(NGLLX,NGLLZ,nspec_elastic_b))
 ! endif !---------------------------------------
...
!! DK DK March 2018: this was missing in Quentin Brissaud's new variational formulation for viscoacoustic media; I added it
  ...
  !displ_elastic_lucas(:,:,:) = 0._CUSTOM_REAL  ! lucas, added the first dimensional for nstep
  !displ_elastic_old_lucas(:,:,:) = 0._CUSTOM_REAL
  !veloc_elastic_lucas(:,:,:) = 0._CUSTOM_REAL
  !accel_elastic_lucas(:,:,:) = 0._CUSTOM_REAL

  !if (SIMULATION_TYPE == 3 .and. any_elastic) then ! lucas, added the first dimensional for nstep
  !  b_displ_elastic_old_lucas(:,:,:) = 0._CUSTOM_REAL
  !  b_displ_elastic_lucas(:,:,:) = 0._CUSTOM_REAL
  !  b_veloc_elastic_lucas(:,:,:) = 0._CUSTOM_REAL
  !  b_accel_elastic_lucas(:,:,:) = 0._CUSTOM_REAL
  !endif
  !----------------------------------------------------------------------------------------------------
 ...
 ! Lucas, CTD-SEM
  if(CTD_SEM .or. Full_Hessian_by_Wavefield_Stored) then
  displ_elastic_m2(:,:) = 0._CUSTOM_REAL  
  displ_elastic_old_m2(:,:) = 0._CUSTOM_REAL
  veloc_elastic_m2(:,:) = 0._CUSTOM_REAL
  accel_elastic_m2(:,:) = 0._CUSTOM_REAL
    if(compute_appro_Hessian) then!-------------! for computing approximate Hessian
      displ_elastic_m1(:,:) = 0._CUSTOM_REAL  
      displ_elastic_old_m1(:,:) = 0._CUSTOM_REAL
      veloc_elastic_m1(:,:) = 0._CUSTOM_REAL
      accel_elastic_m1(:,:) = 0._CUSTOM_REAL
    endif!--------------------------------------

  if (SIMULATION_TYPE == 3 .and. any_elastic) then 
  b_displ_elastic_old_m2(:,:) = 0._CUSTOM_REAL
  b_displ_elastic_m2(:,:) = 0._CUSTOM_REAL
  b_veloc_elastic_m2(:,:) = 0._CUSTOM_REAL
  b_accel_elastic_m2(:,:) = 0._CUSTOM_REAL
  endif
  endif
