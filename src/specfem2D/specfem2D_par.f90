...
! main parameter module for specfem simulations
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: vpext_m2,vsext_m2,rhoext_m2  ! Lucas, CTD-SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: QKappa_attenuationext,Qmu_attenuationext
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: QKappa_attenuationext_m2,Qmu_attenuationext_m2 ! lucas, CTD-SEM
...
!  double precision, dimension(:), allocatable  :: Qmu_attenuation_m2  ! lucas, CTD-SEM with attenuation, not exitiing and used
  ...
  ! material
  ! density
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhostore,rhostore_m2 !lucas, CTD-SEM 
  ! isotropic moduli
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: kappastore,mustore,kappastore_m2,mustore_m2 ! lucas, CTD-SEM
  ! anisotropic
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: &
    c11store,c12store,c13store,c15store,c23store,c25store,c33store,c35store,c55store

  ! for absorbing boundaries
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_vp,rho_vs,rho_vp_m2,rho_vs_m2 ! lucas, CTD-SEM
  ...
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: rmass_inverse_elastic_m2 !lucas, CTD-SEM

  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: accel_elastic,veloc_elastic,displ_elastic  
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: accel_elastic_m1,veloc_elastic_m1,displ_elastic_m1 !lucas, CTD-SEM, for computing approximate Hessian, engine C, GJI 2020
  ! PML/attenuation
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: displ_elastic_old 
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: displ_elastic_old_m1  !lucas, CTD-SEM, for computing approximate Hessian, engine C, GJI 2020

  !lucas, CTD-SEM---------------
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: accel_elastic_m2,veloc_elastic_m2,displ_elastic_m2  
  ! PML/attenuation
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: displ_elastic_old_m2 
  !-----------------------------

  !------------------------------------------------------------------------------------------------------------------
  !lucas: for the volume wave field with time
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: accel_elastic_lucas,veloc_elastic_lucas,displ_elastic_lucas  
  ! PML/attenuation
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: displ_elastic_old_lucas
  !-------------------------------------------------------------------------------------------------------------------
...
!  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: b_dux_dxl_old_m2,b_duz_dzl_old_m2,b_dux_dzl_plus_duz_dxl_old_m2
  ...
  ! lucas, CTD-SEM
    real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: &
    b_accel_elastic_m2,b_veloc_elastic_m2,b_displ_elastic_m2,b_displ_elastic_old_m2  

  !-------------------------------------------------------------------------------------------------------------------------
  !lucas: for the volume wavefield with time
  !  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: &
  !  b_accel_elastic_lucas,b_veloc_elastic_lucas,b_displ_elastic_lucas,b_displ_elastic_old_lucas 
  !-------------------------------------------------------------------------------------------------------------------------
 ...
 ! real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: no_backward_displ_buffer_adj_um1 ! lucas
  ...
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: no_backward_accel_buffer_fwd_du ! lucas
 ...
 ! real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_m2, mu_kl_m2, kappa_kl_m2 !lucas, CTD_SEM, for test only
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_Ha, mu_kl_Ha, kappa_kl_Ha !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_Hbm, mu_kl_Hbm, kappa_kl_Hbm !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_Hbs, mu_kl_Hbs, kappa_kl_Hbs !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_Hc, mu_kl_Hc, kappa_kl_Hc !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rho_kl_Habc, mu_kl_Habc, kappa_kl_Habc !lucas, CTD_SEM

  real(kind=CUSTOM_REAL), dimension(:), allocatable :: mu_k, kappa_k,rho_k
 ! real(kind=CUSTOM_REAL), dimension(:), allocatable :: mu_k_m2, kappa_k_m2,rho_k_m2 !lucas, CTD-SEM
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: mu_k_Ha, kappa_k_Ha,rho_k_Ha !lucas, CTD-SEM  
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: mu_k_Hbm, kappa_k_Hbm,rho_k_Hbm !lucas, CTD-SEM
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: mu_k_Hbs, kappa_k_Hbs,rho_k_Hbs !lucas, CTD-SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl, beta_kl, alpha_kl
 ! real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_m2, beta_kl_m2, alpha_kl_m2 !lucas, CTD_SEM, for test only
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_Ha, beta_kl_Ha, alpha_kl_Ha !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_Hbm, beta_kl_Hbm, alpha_kl_Hbm !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_Hbs, beta_kl_Hbs, alpha_kl_Hbs !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_Hc, beta_kl_Hc, alpha_kl_Hc !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: rhop_kl_Habc, beta_kl_Habc, alpha_kl_Habc !lucas, CTD_SEM
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: source_adjoint_m2 !lucas, used for approximate Hessian under m1
  ...
  ! lucas, CTD-SEM-----------
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: &
    b_absorb_elastic_left_m2,b_absorb_elastic_right_m2,b_absorb_elastic_bottom_m2,b_absorb_elastic_top_m2 ! lucas, CTD-SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: &
    b_absorb_elastic_left_m1,b_absorb_elastic_right_m1,b_absorb_elastic_bottom_m1,b_absorb_elastic_top_m1 ! lucas, CTD-SEM, for approximate Hessian
  ! acoustic
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable ::  &
  ...
  double precision, dimension(:,:,:), allocatable :: sisux_m2,sisuz_m2,siscurl_m2 !lucas, CTD-SEM
...
! parameter module for noise simulations
  ...
  double precision, dimension(:,:), allocatable :: vector_field_display_m2 !lucas, CTD-SEM
  double precision, dimension(:,:), allocatable :: vector_field_display_m1 !lucas, CTD-SEM
  ...
  double precision, dimension(:), allocatable :: data_pixel_recv_m2 !lucas, CTD-SEM
  double precision, dimension(:), allocatable :: data_pixel_recv_m1 !lucas, CTD-SEM
  double precision, dimension(:), allocatable :: data_pixel_send
  double precision, dimension(:), allocatable :: data_pixel_send_m2 !lucas, CTD-SEM
  double precision, dimension(:), allocatable :: data_pixel_send_m1 !lucas, CTD-SEM
  ...
  double precision, dimension(:,:), allocatable :: image_color_data_m2 !lucas, CTD-SEM
  double precision, dimension(:,:), allocatable :: image_color_data_m1 !lucas, CTD-SEM
  double precision, dimension(:,:), allocatable :: image_color_vp_display
  double precision, dimension(:,:), allocatable :: image_color_vp_display_m2 !lucas, CTD-SEM
  double precision, dimension(:,:), allocatable :: image_color_vp_display_m1 !lucas, CTD-SEM
