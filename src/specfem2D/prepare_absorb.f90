...
!--------------lucas, CTD-SEM------------------------------------------
subroutine prepare_absorb_read_elastic_m2()
...
! reads in previously stored Stacey boundary contributions
  ...
  if (nspec_left > 0) then
    ...
    open(unit=350,file=trim(OUTPUT_FILES)//outputname,status='old',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening absorbing boundary file')
    ! reads in boundary contributions
    read(350) b_absorb_elastic_left_m2
    close(350)
  endif
  ...
  if (nspec_right > 0) then
    ...
    if (ier /= 0) call exit_MPI(myrank,'Error opening absorbing boundary file')
    ! reads in boundary contributions
    read(350) b_absorb_elastic_right_m2
    close(350)
  endif
  ...
  if (nspec_bottom > 0) then
    ...
    if (ier /= 0) call exit_MPI(myrank,'Error opening absorbing boundary file')
    ! reads in boundary contributions
    read(350) b_absorb_elastic_bottom_m2
    close(350)
  endif
  ...
  if (nspec_top > 0) then
    ...
    if (ier /= 0) call exit_MPI(myrank,'Error opening absorbing boundary file')
    ! reads in boundary contributions
    read(350) b_absorb_elastic_top_m2
    close(350)
  endif

  end subroutine prepare_absorb_read_elastic_m2
