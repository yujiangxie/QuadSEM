...
! therefore not be consistent with the basis functions.
  ...
  ! reads in parameters
  call initialize_simulation() ! lucas: not very important set with respect to wave fields outputed.

  ! reads sources, stations, and mesh from database
  call read_mesh_databases() ! lucas, read_materials () set here, some initial material also set here

  ! sets up reference element GLL points/weights/derivatives
  call setup_GLL_points()

  ! sets up global mesh numbering and mesh properties
  call setup_mesh() ! lucas, read_exteral_model () set here, ! lucas: to get vpext(i,j,ispec) and vpext_m2(i,j,ispec), etc. here, CTD-SEM alread implemented. 
