...
! reads in material definitions in DATA/Par_file
  ...
  do imaterial = 1,nbmodels
    ...
    call read_material_parameters_p(i,icodematread, &
                                    ...
                                    val8read,val9read,val10read,val11read,val12read) ! lucas, i=material number (no.)  icodematread=material_type, then 0 to 12 values.

    ! checks material id
    if (i < 1 .or. i > nbmodels) call stop_the_code('Wrong material number!')
    icodemat(i) = icodematread ! lucas, icodematread is the second number in each line below nbmodels

    ! sets material properties
    if (icodemat(i) == ISOTROPIC_MATERIAL) then ! lucas:ISOTROPIC_MATERIAL =1
      ! isotropic materials
    ...
    else if (icodemat(i) == ANISOTROPIC_MATERIAL) then ! lucas: ANISOTROPIC_MATERIAL=2
    ...
    else if (icodemat(i) == POROELASTIC_MATERIAL) then ! lucas: POROELASTIC_MATERIAL=3
    ...
    else if (icodemat(i) <= 0) then ! lucas: this is used for reading external tomography, where icodemat(i)=-1
...
'Just one material can be defined by a tomo file for now (we would need to write a nummaterial_velocity_file)')
      ...
      !-----lucas changed below-------------
      rho_s_read(i) = val0read ! -1.0d0  
      cp(i) = val1read !-1.0d0 
      cs(i) = val2read
      QKappa(i) = val5read! -1.0d0
      Qmu(i) = val6read  !-1.0d0
      aniso3(i) = val3read ! -1.0d0    
      aniso4(i) = val3read ! -1.0d0
      !-----lucas changed above------------

      if (abs(cs(i)) > TINYVAL) then
        phi_read(i) = 0.d0           ! elastic   ! lucas, 
      else
        phi_read(i) = 1.d0           ! acoustic
      endif
