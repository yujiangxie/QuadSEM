...
!-------------------------------------------------------------------------------
  ...
  ! ' CTD-SEM'--------stored or not, where CTD_SEM means without store---------------
  write(IOUT) CTD_SEM
  write(IOUT) compute_appro_Hessian
  write(IOUT) Full_Hessian_by_Wavefield_Stored
  write(IOUT) TOMOGRAPHY_FILE_m2
