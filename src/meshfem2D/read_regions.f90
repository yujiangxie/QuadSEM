...
! reads in material definitions in DATA/Par_file and outputs to num_material
  ...
  do iregion = 1,nbregions
    ...
    if (icodemat(imaterial_number) /= ANISOTROPIC_MATERIAL .and. icodemat(imaterial_number) /= POROELASTIC_MATERIAL) then  ! where icodemat indicates which material (1,1,2,3,-1) applied.

       ! isotropic material
       vpregion = cp(imaterial_number) ! lucas: vp 
       vsregion = cs(imaterial_number) ! lucas: vs
       ...
       write(IMAIN,*) 'rho    = ',sngl(rho_s_read(imaterial_number)) ! lucas: rho
       ...
       write(IMAIN,*) 'QKappa = ',sngl(QKappa(imaterial_number)) ! lucas: QKappa
       write(IMAIN,*) 'Qmu    = ',sngl(Qmu(imaterial_number))    ! lucas: Qmu
