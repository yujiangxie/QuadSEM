...
! reads in DATA/Par_file
  ...
  if (myrank == 0) then
    ! opens file Par_file
    call open_parameter_file() ! ! lucas: just open and do nothing related to vp, vs...
    ...
    call read_material_table() ! lucas, got the 15 material parameters set in Par_file, also also the cp, cs, rho_s_read, Qkappa, Qmu are set within based on the vp, vs, rho in Par_file.
    ...
    if (imesher == 1 .and. (.not. read_external_mesh) ) then
      ...
      call read_regions() ! lucas, here cp, cs, rho_s_read, Qkappa, Qmu already got.
    endif
...
! if this parameter exists in the Par_file
  ...
  !--------------------------------------------------------------------
  ! CTD-SEM method, stored the wavefield or not, if not, use the CTD_SEM, otherwise, use the
  ! Full_Hessian_by_Wavefield_Stored
  !--------------------------------------------------------------------
  call read_value_logical_p(CTD_SEM, 'CTD_SEM')
  if (err_occurred() /= 0) then
    some_parameters_missing_from_Par_file = .true.
    write(*,'(a)') 'CTD_SEM                            = .true.'
    write(*,*)
  endif

  call read_value_logical_p(compute_appro_Hessian, 'compute_appro_Hessian')
  if (err_occurred() /= 0) then
    some_parameters_missing_from_Par_file = .true.
    write(*,'(a)') 'compute_appro_Hessian                            = .true.'
    write(*,*)
  endif

  call read_value_logical_p(Full_Hessian_by_Wavefield_Stored, 'Full_Hessian_by_Wavefield_Stored')
  if (err_occurred() /= 0) then
    some_parameters_missing_from_Par_file = .true.
    write(*,'(a)') 'Full_Hessian_by_Wavefield_Stored                            = .true.'
    write(*,*)
  endif

  call read_value_string_p(TOMOGRAPHY_FILE_m2, 'TOMOGRAPHY_FILE_m2')
  if (err_occurred() /= 0) then
    some_parameters_missing_from_Par_file = .true.
    write(*,'(a)') 'TOMOGRAPHY_FILE_m2                 = ./DATA/tomo_file.xyz'
    write(*,*)
  endif
