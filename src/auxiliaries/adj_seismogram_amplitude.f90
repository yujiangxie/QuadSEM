...
!--------------------------------------------------------------------------------
  ...
  do irec = 1,nrec
    ...
    do icomp = 1,NDIMr ! lucas NDIMr=2 when chosen adj_comp: 3

      filename = 'OUTPUT_FILES_SEM/'//trim(station_name(irec))//'.'// compr(icomp) // '.semd'   ! lucas: compr = (/"BXX","BXZ"/) when adj_comp=3
  ...
  do irec = 1,nrec
    ...
    do icomp = 1,NDIMr
      ...
      filename = 'OUTPUT_FILES_SEM/'//trim(station_name(irec))//'.'// compr(icomp) // '.semd'
      open(unit = 10, file = trim(filename),status='old',iostat=ier)
      if (ier /= 0) then
        print *,'Error opening file: ',trim(filename)
      ...
      ! reads in trace
      do itime = 1,NSTEP
        read(10,*) time , seism(itime,icomp) ! lucas, keep in mind the itime
      enddo
      close(10)
    enddo
